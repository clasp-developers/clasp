...
namespace core
{
    ...
    T_sp StreamByteStack(T_sp strm)
    {
        Stream_sp stream = strm.as<Stream_O>();
 ...
 */
	...
	if (out != EOF) clasp_unread_char(out, strm);
	return out;
    }
...
#ifdef ECL_UNICODE
	T_sp s = extended?
            clasp_alloc_adjustable_extended_string(line_length) :
            clasp_alloc_adjustable_base_string(line_length);
#else
 ...
 */
    ...
    T_sp
    clasp_make_string_input_stream(T_sp strng, cl_index istart, cl_index iend)
    {
	T_sp strm;
 ...
 */
    ...
    T_sp
    cl_make_synonym_stream(T_sp tsym)
    {
	T_sp x;
...
#define CONTROL_Z 26
	...
	T_sp output;
	if (isatty(desc)) {
            output = clasp_make_stream_from_FILE
                (fname,
	...
	if (isatty(desc)) {
            ...
            output = clasp_make_file_stream_from_fd
                (fname, desc, smm,
                 byte_size, flags,
                 external_format);
	}
...
#define maybe_make_windows_console_fd clasp_make_file_stream_from_fd
    ...
    int
    clasp_stream_to_handle(T_sp s, bool output)
    {
    BEGIN:
...
# endif
    ...
    T_sp
    clasp_read_byte(T_sp strm)
    {
	return stream_dispatch_table(strm).read_byte(strm);
    }

    void
    clasp_write_byte(T_sp c, T_sp strm)
    {
 ...
 */
	...
	if (flags & CLASP_STREAM_C_STREAM) {
            ...
            switch (smm) {
            case clasp_smm_probe:
            case clasp_smm_input: fp = safe_fopen(fname.c_str(), OPEN_R); break;
            case clasp_smm_output:
            ...
            output = clasp_make_stream_from_FILE(fn, fp, smm, byte_size, flags,
                                               external_format);
            si_set_buffering_mode(output, byte_size? kw::_sym_full : kw::_sym_line);
	} else {
...
namespace core {
    ...
    T_sp IOFileStream_O::make(const string& name
                            , int fd
                            , enum StreamMode smm
                            , T_sp elementType
                            , T_sp externalFormat )
...
#define DOCS_cl_peekChar "peekChar"
    ...
    HANDLE_EOF:
	if ( eof_errorp.isTrue() ) ERROR_END_OF_FILE(strm);
	return eof_value;
    }
...
#define ARGS_cl_terpri "(&optional (output-stream ext:+process-standard-output+))"
