...
#define SUBIMP() _G();SUBCLASS_MUST_IMPLEMENT();
...
#ifdef DEBUG_ASSERTS
#define HARD_ASSERT(t) if (!(t)) {core::errorFormatted("HARD_ASSERT failed");throw(core::HardError(__FILE__,__FUNCTION__,__LINE__,"Assertion " #t " failed"));};
#define HARD_ASSERTF(t,fmt) if (!(t)) {core::errorFormatted(fmt);throw(core::HardError(__FILE__,__FUNCTION__,__LINE__,BF("Assertion %s failed: %s") % #t % (fmt).str() ));};
#else
...
#define lisp_LOG(___fmt) {if (core::lisp_debugIsOn(__FILE__)) { core::lisp_debugLogWrite(__FILE__,__FUNCTION__,__LINE__,0,___fmt); }}
#define LOG(___fmt) lisp_LOG(___fmt)

#define lisp_SHOUT(___fmt) {if (core::lisp_debugIsOn(__FILE__,DEBUG_SHOUT)) { core::lisp_debugLogWrite(__FILE__,__FUNCTION__,__LINE__,0,___fmt); }}
#define SHOUT(___fmt) lisp_SHOUT(___fmt)

#define	IF_DEBUG_ON(f)	f
#define	DEBUG_ASSERT(x)	if (!(x)) {stringstream ss; ss<<"Assertion failed file("<<__FILE__<<") line("<<__LINE__<<")"; SIMPLE_ERROR(BF("%s")%ss.str());};
#define	DEBUG_ASSERTP(x,e)	if (!(x)) {stringstream ss; ss<<"Assertion failed ("<<e<<" file("<<__FILE__<<") line("<<__LINE__<<")"; SIMPLE_ERROR(BF("%s") % ss.str());};
...
#ifdef	DEBUG_ASSERTS
#define lisp_ASSERT(l,x) if (!(x)) {SIMPLE_ERROR(BF("Assertion [%s]  failed file(%s) line(%s)") % #x % __FILE__ % __LINE__);}
#define	ASSERT(x) lisp_ASSERT(_lisp,x)
#define	lisp_ASSERTP(l,x,e)	if (!(x)) {SIMPLE_ERROR(BF("%s")%(e));}
#define ASSERTP(x,e) lisp_ASSERTP(_lisp,x,e)
#define lisp_ASSERTF(l,x,f) if (!(x)) {SIMPLE_ERROR(f);};
#define ASSERTF(x,f) lisp_ASSERTF(_lisp,x,f)
#define	ASSERT_eq(x,y)	if (!(x==y)) {SIMPLE_ERROR(BF("Assertion [%s==%s] failed values are (%s) and (%s)") % #x % #y % (x) % (y));}
#define	ASSERT_lessThan(x,y)	if (!(x<y)) {SIMPLE_ERROR(BF("Assertion [%s<%s] failed values are (%s) and (%s)") % #x % #y % (x) % (y));}
#define ASSERT_lt(x,y) ASSERT_lessThan(x,y)
#define	ASSERT_greaterThan(x,y)	if (!(x>y)) {SIMPLE_ERROR(BF("Assertion [%s>%s] failed values are (%s) and (%s)") % #x % #y % (x) % (y));}
#define	ASSERT_gt(x,y) ASSERT_greaterThan(x,y)
#define	FATAL_ASSERTP(x,e) if (!(x)) {SIMPLE_ERROR(_lisp,core::LispError_O::create(BF("Assertion [%s] failed - %s") % #x % (e),_lisp ));};

#if DEBUG_TRAP_NULLS==1
#define ASSERTNOTNULLP(x,e) if (!(x)) {SIMPLE_ERROR(BF("Assertion failed! (shared_ptr: %s is NULL) %s") % #x % (e));};
#else
...
#define _lisp_BLOCK_TRACEF(__f)
        ...
        void af_readerError(const string& sourceFile, uint lineno, Symbol_sp function,
                            Str_sp fmt, Cons_sp fmtargs, Stream_sp stream=_Nil<Stream_O>() );
