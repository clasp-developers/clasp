...
static void debugger_dump_current_module(DebuggerFrame_sp cur, int index) {
  ...
  EntryPoint_sp ep = func->entryPoint();
  if (gc::IsA<GlobalBytecodeEntryPoint_sp>(ep)) {
    Array_sp bytecode = gc::As<Array_sp>(gc::As_unsafe<GlobalBytecodeEntryPoint_sp>(ep)->code());
...
T_mv early_debug_inner(DebuggerFrame_sp bot, bool can_continue) {
  ...
  DynamicScopeManager tempBind2(comp::_sym_STARimplicit_compile_hookSTAR, comp::_sym_implicit_compile_hook_default);
