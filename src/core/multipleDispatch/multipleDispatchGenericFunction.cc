...
//      arg = translate::from_object<XXXX>::convert(env->lookup(this->Package(),"YYY"));
	...
	LOG(BF("defmethod for symbol[%s] called with method with receiverClass[%s]")
	    % name->__repr__() % method->receiver_class()->__repr__() );
    ...
    */
	...
	LOG(BF("Invoked MultipleDispatchGenericFunction[%s] with receiver class[%s]")
	    % this->getSymbol()->__repr__() % dispatchArgClass->__repr__() );
	Function_sp emf =
	...
	{
	    THROW(_lisp->error(BF("There are no applicable methods of %s for receiver class %s")
			       % this->getSymbol()->__repr__()
