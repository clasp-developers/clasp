...
namespace core {
static void write_array_inner(bool vector, Array_sp x, T_sp stream) {
...
#define DEFWRITESV(type)\
  void type::__write__(T_sp stream) const {\
    if (!clasp_print_array() && !clasp_print_readably())\
      write_simple_vector_simple("#<" #type " ", this->asSmartPtr(), stream);\
    else write_array_inner(true, this->asSmartPtr(), stream);\
  }

DEFWRITESV(SimpleVector_byte8_t_O)
DEFWRITESV(SimpleVector_byte16_t_O)
DEFWRITESV(SimpleVector_byte32_t_O)
DEFWRITESV(SimpleVector_byte64_t_O)
DEFWRITESV(SimpleVector_fixnum_O)
DEFWRITESV(SimpleVector_int8_t_O)
DEFWRITESV(SimpleVector_int16_t_O)
DEFWRITESV(SimpleVector_int32_t_O)
DEFWRITESV(SimpleVector_int64_t_O)
