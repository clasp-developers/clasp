...
(defun do-setf-method-expansion (name lambda args)
  ...
  (let* ((vars '())
	...
	(push item inits)
	(setq item (gensym "do-setf-method-expansion"))
	(push item vars))
      (push item all))
    (let* ((store (gensym "store-do-setf-method-expansion"))
	   (all (nreverse all)))
...
;; Setup the system defsetf update functions
;; These are stored in *system-defsetf-update-functions* as (list access-fn update-fn) pairs
#|
(dolist (pair *system-defsetf-update-functions*)
  (defsetf (car pair) (cdr pair)))
|#
