      ...
      (progn
        ...
        (let ((stamp-max 0)
          ...
          (dolist (c sorted-classes)
            (format sout "STAMP_~a = ~a,~%" (mangle-key (class-key% c)) (stamp% c))
            (setf stamp-max (max stamp-max (stamp% c))))
          (maphash (lambda (key type)
                     (format sout "STAMP_~a = ~a,~%" (mangle-key (c++type% type)) (stamp% type))
        ...
        (flet ((write-one-gcstamp (type mangled-key flags)
          ...
          (dolist (c sorted-classes)
            (write-one-gcstamp (class-key% c) (mangle-key (class-key% c)) (string (flags% c))))
          (maphash (lambda (key type)
                     (write-one-gcstamp (c++type% type) (mangle-key (c++type% type)) "FLAGS_STAMP_IN_HEADER"))
                   gc-managed-types))
