...
(print (list* :quaviver/boot *features*)
       (open (core:argv 3) :if-exists :overwrite :if-does-not-exist :create :direction :output))
          ...
          ,@(let ((ss '()))
              (flet ((frob (k v)
                ...
                (mapc #'(lambda (p)
                        ...
                        (list-all-packages)))
              ss)
           (setf ,@(mapcan #'(lambda (q)
                               (mapcan #'(lambda (pair)
                                            `((get ',(car q) ,(car pair)) ',(cdr pair)))
                                       (cdr q)))
                           (core:float-traits))))
