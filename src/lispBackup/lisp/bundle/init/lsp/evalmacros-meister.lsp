...
#|  ;; multiple-value-bind is defined just before defmacro
...
(dbg-msg "About to export when")
(export 'when)

(dbg-msg "About to defmacro unless")
...
last FORM.  If not, simply returns NIL."
  (if (not body) (error "unless body is nil!!!!!"))
...
(dbg-msg "About to defmacro defparameter")
...
(dbg-msg "About to defun function-block-name")

(defun function-block-name (name)
  (if (symbolp name)
      name
      (error "Handle argument[~a] in function-block-name - see ecl/compiler.d/function-block-name" name )))
...
(defmacro lambda-block (name lambda-list &rest lambda-body)
  (multiple-value-bind (decl body doc)
      (process-declarations lambda-body)
...
; conditionals
#|
;; Use the cond macro below that replaces dolist with tagbody loop
...
(export '(multiple-value-list multiple-value-setq))
