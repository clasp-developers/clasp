...
#|
#+cando(si::fset 'bq #'(lambda (whole env) 
			 (let ((x (cadr whole)))
			   (bq-completely-process x)))
		 1)
#-cando(defmacro bq (x) (bq-completely-process x))
(export 'bq)
|#
...
(defun bq-completely-process (x) 
  (let ((raw-result (backquote-process x)))
    (backquote-remove-tokens (if *bq-simplify* 
                          (backquote-simplify raw-result) 
                          raw-result))))


(defparameter *backquote-expand-hook* #'backquote-completely-process)
(defun switch-backquote (x)
  (if (eq x 'lisp)
      (setq *backquote-expand-hook* #'bq-completely-process)
      (setq *backquote-expand-hook* #'backquote-completely-process)))


#|
(defun bq-process (x) 
  (let ((process-result (cond ((atom x) 
			       (bformat t "\n(atom x) --> true")
			       (list *bq-quote* x)) 
			      ((eq (car x) 'backquote) 
			       (bformat t "\n(eq (car x) 'backquote) --> true")
			       (backquote-process (backquote-completely-process (cadr x)))) 
			      ((eq (car x) *comma*)
			       (bformat t "\n(eq (car x) *comma*) --> true")
			       (cadr x)) 
			      ...
			      (t (do ((p x (cdr p)) 
				      (q '()
					 (progn
					   (bformat t "\n calling backquote-bracket with: %s" (car p))
					   (let ((bracketed (backquote-bracket (car p)) ))
					     (bformat t "\n Came out of backquote-bracket with: %s" bracketed)
					     (cons bracketed q))))
				      (step 0 (+ 1 step))) 
				     ((atom p) 
				      (let ((result (cons *bq-append* 
							  (nreconc q (list (list *bq-quote* p))))))
					result))
				   (bformat t "\n---backquote-process----- step[%d]" step)
				   (bformat t "\n p--> %s" p)
				   (bformat t "\n q--> %s" q)
				   (if (eq (car p) *comma*) 
				       (progn
					 ...
					 (bformat t "\n(eq (car p) *comma*) --> true")
					 (let ((res (cons *bq-append* 
							  (nreconc q (list (cadr p))))))
					   (bformat t "\nReturning from comma")
					   res)))
    ...
    (bformat t "\n Falling out of backquote-process")
    process-result
  ))
|#
...
(defun bq-simplify (x) 
  (if (atom x) 
      ...
      (let ((x (if (eq (car x) *bq-quote*) 
                   x 
                   (backquote-maptree #'bq-simplify x)))) 
        (if (not (eq (car x) *bq-append*)) 
            x 
            (backquote-simplify-args x)))))
|#
...
(defun bq-simplify-args (x) 
  (do ((args (reverse (cdr x)) (cdr args)) 
	 ...
	 (progn
	   (cond ((atom (car args)) 
		  (backquote-attach-append *bq-append* (car args) result)) 
		 ((and (eq (caar args) *bq-list*) 
		       (notany #'bq-splicing-frob (cdar args))) 
		  (backquote-attach-conses (cdar args) result)) 
		 ((and (eq (caar args) *bq-list**) 
		       (notany #'bq-splicing-frob (cdar args))) 
		  (backquote-attach-conses 
		   (reverse (cdr (reverse (cdar args)))) 
		   (backquote-attach-append *bq-append* 
				     (car (last (car args))) 
				     result))) 
		 ((and (eq (caar args) *bq-quote*) 
		       ...
		       (null (cddar args))) 
		  (backquote-attach-conses (list (list *bq-quote* 
						(caadar args))) 
				    result)) 
		 ((eq (caar args) *bq-clobberable*) 
		  (backquote-attach-append *bq-nconc* (cadar args) result)) 
		 (t
		  (backquote-attach-append *bq-append* 
                                    (car args) 
                                    result)))))) 
...
(defun bq-remove-tokens (x) 
  (cond ((eq x *bq-list*) 'list) 
        ...
        ((and (eq (car x) *bq-list**) 
              (consp (cddr x)) 
              (null (cdddr x))) 
         (cons 'cons (backquote-maptree #'bq-remove-tokens (cdr x)))) 
        (t
	 (let ((mapped (backquote-maptree #'bq-remove-tokens x) ))
	   mapped))
	))
|#
