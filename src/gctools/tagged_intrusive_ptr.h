...
template<class T> class tagged_intrusive_ptr
{
private:
...
//    tagged_intrusive_ptr x((A*)this);
    ...
    template<class U> tagged_intrusive_ptr & operator=(tagged_intrusive_ptr<U> const & rhs)
    {
...
template<class T, class U> inline bool operator==(tagged_intrusive_ptr<T> const & a, tagged_intrusive_ptr<U> const & b)
{
    if ( !(a.pointerp() && b.pointerp()) ) {
        ...
        if ( a.unboundp() || b.unboundp() ) return false;
...
template<class T, class U> inline bool operator!=(tagged_intrusive_ptr<T> const & a, tagged_intrusive_ptr<U> const & b)
{
    if ( !(a.pointerp() && b.pointerp()) ) {
        ...
        if ( a.unboundp() || b.unboundp() ) return true;
        THROW_HARD_ERROR(BF("Implement more != comparisons for tagged ptrs"));
    }
    return (dynamic_cast<void*>(a.pxget()) != dynamic_cast<void*>(b.pxget()));
}
...
template<class T> void swap(tagged_intrusive_ptr<T> & lhs, tagged_intrusive_ptr<T> & rhs)
{
    lhs.swap(rhs);
}
...
template<class T> T * get_pointer(tagged_intrusive_ptr<T> const & p)
{
    return p.get();
}

template<class T, class U> tagged_intrusive_ptr<T> static_pointer_cast(tagged_intrusive_ptr<U> const & p)
{
    return static_cast<T *>(p.get());
}

template<class T, class U> tagged_intrusive_ptr<T> const_pointer_cast(tagged_intrusive_ptr<U> const & p)
{
    return const_cast<T *>(p.get());
}

template<class T, class U> tagged_intrusive_ptr<T> dynamic_pointer_cast(tagged_intrusive_ptr<U> const & p)
{
...
template<class Y> std::ostream & operator<< (std::ostream & os, tagged_intrusive_ptr<Y> const & p)
{
    os << p.get();
    return os;
}
...
template<class E, class T, class Y> basic_ostream<E, T> & operator<< (basic_ostream<E, T> & os, tagged_intrusive_ptr<Y> const & p)
# else
template<class E, class T, class Y> std::basic_ostream<E, T> & operator<< (std::basic_ostream<E, T> & os, tagged_intrusive_ptr<Y> const & p)
# endif 
{
    os << p.get();
    return os;
...
template< class T > std::size_t hash_value( boost::tagged_intrusive_ptr<T> const & p )
{
