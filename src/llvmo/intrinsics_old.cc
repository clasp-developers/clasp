...
extern "C" {
	...
	if ( argIdx >= nargs ) return;
        throw(UnrecognizedKeywordArgumentError(argArray[argIdx]);
#if 0
	stringstream ss;
...
extern "C"
{
	...
	*destP = *sourceP;
    ...
    {_G();
	(*resultConsP) = core::Cons_O::create(*carP,*cdrP,_lisp);
    }
	...
	core::T_mv activate(core::const_ActivationFrame_spREF closedEnv,int nargs, ArgArray args)
	{
	    core::T_mv result;
...
//	IMPLEMENT_MEF(BF("Figure out what to do in this case"));
	...
	*resultP = Values((*symP)->symbolValue());
    ...
    {_G();
	ASSERT(frameP!=NULL);
	return &((*frameP)->lookupValueReference(depth,index));
    }
...
{_G();
    ...
    core::T_sp res = (*renvP)->lookupValue(depth,index);
    ASSERTF(*renvP,BF("lexicalValueRead is NULL depth[%d] index[%d] activationFrame: %s") % depth % index % _rep_((*renvP)));
    return res;
}
...
extern "C"
{
	...
	core::T_sp* pos = &frame->entryReference(idx);
	return pos;
    }
	...
	core::ValueFrame_sp frame = (*frameP).pointerAsUnsafe<core::ValueFrame_O>();
	core::T_sp* pos = &frame->entryReference(ridx);
	return pos;
    }
	...
	{
	    result = core::Cons_O::create(frame->entry(i),result,_lisp);
	}
	(*restP) = result;
	ASSERTNOTNULL(*restP);
    }
...
{_G();
    ...
    {
	result = core::Cons_O::create(mv,result,_lisp);
	for ( int i=1; i<mv.number_of_values(); i++ )
	{
	    result = core::Cons_O::create(mv.valueGet(i),result,_lisp);
	}
    }
    return result;
}
...
/*! Return a pointer to the shared_ptr of the activation frames parent */
    ...
    {_G();
	return &_lisp->nilActivationFrame();
    }
...
extern "C"
{
	...
	} else
	{
	    ...
	    {
		dims = core::Cons_O::create(core::Fixnum_O::create(dimensions[i]),dims,_lisp);
	    }
...
//	printf("intrinsics.cc loadValues vo->length() = %d\n", vo->length() );
    ...
    */
	...
	if ( !af_keywordP((*objP)) )
	{
            throw(UnrecognizedKeywordArgumentError(*objP));
	}
    }
