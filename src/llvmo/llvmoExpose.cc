...
#if 1
  printf("%s:%d:%s I HAVE A PROBLEM HERE - WHAT DO I DO WITH replPtrRaw %p - I need a GlobalEntryPoint_O\n",
         __FILE__, __LINE__, __FUNCTION__, replPtrRaw );
  abort();
#else  
  core::ClaspXepGeneralFunction lisp_funcPtr = (core::ClaspXepGeneralFunction)(replPtrRaw);
  StartupFunctions* startups = new StartupFunctions(lisp_funcPtr);
  core::ClaspXepFunction startupXep();
