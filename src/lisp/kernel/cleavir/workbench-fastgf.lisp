...
(let ((CLOS::*TRAP* (cons t nil)))
  ...
  (core:fmt t "*trap-recursive-dispatch-miss* -> {}%N" clos::*trap*)
...
(print "Starting")
(time (compile-file "sys:src;lisp;modules;asdf;build;asdf.lisp" :print t))
(progn ;; Set up everything for building cclasp from bclasp with auto-compile
  (format t "Loading ASDF system~%")
  (finish-output)
  (time (load "sys:src;lisp;modules;asdf;build;asdf.fasl"))
...
(let ((clos::*monitor-dispatch* t)
      (clos::*dispatch-log* nil))
  (clasp-cleavir:cleavir-compile-file #P"sys:src;lisp;kernel;lsp;foundation.lisp" :print t)
  (print "------- dispatch-log----")
