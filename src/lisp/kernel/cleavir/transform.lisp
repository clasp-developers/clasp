...
(defmacro define-vector-transforms (element-type)
  `(progn
     ...
     (deftransform (setf aref) (((val ,element-type)
                                 (arr (simple-array ,element-type (*)))
                                 (index t))
                                :policy policy)
       (list 'setf (list 'core:vref 'arr (bounds-check-form 'arr 'index policy)) 'val))
     (deftransform row-major-aref (((arr (simple-array ,element-type (*))) (index t))
                                   :policy policy)
       (list 'core:vref 'arr (bounds-check-form 'arr 'index policy)))
     (deftransform (setf row-major-aref) (((val ,element-type)
                                           (arr (simple-array ,element-type (*)))
                                           (index t))
                                          :policy policy)
