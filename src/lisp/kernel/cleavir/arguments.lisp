...
(defun gather-required-arguments (reqargs
  ...
  (let (reqs)
    (do* ((cur-req (cdr reqargs) (cdr cur-req))
	 ...
	 ((endp cur-req) (cmp:irc-store arg-idx arg-idx-alloca))
      (let* ((target-alloca (translate-datum target))
	     (val-ref (cmp:calling-convention-args.gep args arg-idx)))
	(cmp:irc-store (cmp:irc-load val-ref) target-alloca)
...
(defun gather-optional-arguments (optargs
  ...
  (let (opts)
    (do* ((cur-opt (cdr optargs) (cdddr cur-opt))
	 ...
	 ((endp cur-opt) (cmp:irc-store arg-idx arg-idx-alloca))
	...
	(let ((target-alloca (translate-datum target))
	  ...
	  (cmp:irc-low-level-trace :arguments)
	  (cmp:irc-store (cmp:irc-load val-ref) target-alloca)
	  (cmp:irc-store true-val flag-alloca)
	  (cmp:irc-br cont-block)
	  (cmp:irc-begin-block init-block)
	  (cmp:irc-low-level-trace :arguments)
	  (cmp:irc-store (cmp:irc-nil) target-alloca)
	  (cmp:irc-store (cmp:irc-nil) flag-alloca)
	  (cmp:irc-br cont-block)
...
(defun compile-rest-argument (rest-var
  ...
  ;; Copy argument values or evaluate init forms
    (let* ((arg-idx (cmp:irc-load arg-idx-alloca))
      ...
      (cmp:irc-store rest rest-alloca)
      rest-alloca)))
...
(defun compile-key-arguments (keyargs
  ...
  (let ((process-kw-args-block (cmp:irc-basic-block-create "process-kw-args")))
	...
	(cmp:irc-store (cmp:irc-nil) supplied-alloca)))
		...
		(let* ((kw-val (%literal key (string key)))
		  ...
		  (let ((arg-idx+1 (cmp:irc-add phi-arg-idx (%size_t 1)))
		    ...
		    (let ((kw-arg-val (cmp:irc-load (cmp:calling-convention-args.gep args arg-idx+1))))
		      (cmp:irc-store kw-arg-val target-ref)
		      ;; Set the boolean flag to indicate that we saw this key
		      (cmp:irc-store true-val supplied-ref)
...
(defun compile-general-lambda-list-code (reqargs 
  ...
  (let* ((arg-idx-alloca (alloca-size_t "arg-idx-alloca")))
    (cmp:irc-store (%size_t 0) arg-idx-alloca)
	...
	(let ((out (translate-datum (pop outputs))))
	  (cmp:irc-store (cmp:irc-load req) out)))
      (dolist (opt opts)
	(let ((out (translate-datum (pop outputs))))
	  (cmp:irc-store (cmp:irc-load opt) out)))
      (when rest-var
	(cmp:irc-store (cmp:irc-load (translate-datum rest-var)) (translate-datum (pop outputs))))
      (do* ((cur-key (cdr keyargs) (cddddr cur-key))
	...
	(let ((target-output-ref (translate-datum (pop outputs)))
	  ...
	  (cmp:irc-store (cmp:irc-load (translate-datum target)) target-output-ref)
	  (cmp:irc-store (cmp:irc-load (translate-datum supplied)) supplied-output-ref)))
      )))
...
(defun compile-general-lambda-list-code (reqargs 
  ...
  (let* ((arg-idx-alloca (alloca-size_t "arg-idx-alloca"))
	...
	(cmp:irc-store (cmp:irc-load req-alloca) out-alloca)))
	...
	(let ((target-alloca (translate-datum target))
	  ...
	  (cmp:irc-store (cmp:irc-load target-alloca) target-out-alloca)
	  (cmp:irc-store (cmp:irc-load supplied-alloca) supplied-out-alloca))))
    (when rest-var
