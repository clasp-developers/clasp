...
(defun generate-landing-pad-for-unwind (function-info enter-instruction return-value tags abi maybe-unwind-landing-pad not-unwind-exception-bb)
...
(defun generate-landing-pad-for-debug (function function-info resume-block)
  (let* ((cleanup-landing-pad       (cmp:irc-basic-block-create "cleanup-lpad" function))
    ...
    (cmp:with-irbuilder (ehbuilder)
      (let* ((landpad                   (cmp:irc-create-landing-pad 1))
             (_                         (llvm-sys:add-clause landpad (llvm-sys:constant-pointer-null-get cmp:%i8*%)))
    ...
    (let* ((ehresume-block  (generate-ehresume-code function function-info))
      ...
      (when cleanup-block
        (setf (landing-pad-for-debug function-info)
              (generate-landing-pad-for-debug function function-info cleanup-block)))
