...
(defvprimop (core::sf-truncate :flags (:flushable))
    ((:utfixnum :single-float) :single-float :single-float)
  (inst)
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
    (list (%fptosi (%fdiv i1 i2) cmp:%fixnum%)
          (%frem i1 i2))))
(defvprimop (core::df-truncate :flags (:flushable))
    ((:utfixnum :double-float) :double-float :double-float)
  (inst)
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
...
(defvprimop (core::sf-ffloor :flags (:flushable))
    ((:single-float :single-float) :single-float :single-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.floor.f32" (list quotient)))
         ;; LLVM doesn't provide a builtin to get the corresponding remainder.
         ;; so compute the remainder in the most obvious way possible,
         ;; from the equation quotient*divisor + remainder = dividend.
         ;; not sure that's rounded correctly though. FIXME?
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))
(defvprimop (core::df-ffloor :flags (:flushable))
    ((:double-float :double-float) :double-float :double-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.floor.f64" (list quotient)))
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))

(defvprimop (core::sf-fceil :flags (:flushable))
    ((:single-float :single-float) :single-float :single-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.ceil.f32" (list quotient)))
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))
(defvprimop (core::df-fceil :flags (:flushable))
    ((:double-float :double-float) :double-float :double-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.ceil.f64" (list quotient)))
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))

(defvprimop (core::sf-froundeven :flags (:flushable))
    ((:single-float :single-float) :single-float :single-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.roundeven.f32" (list quotient)))
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))
(defvprimop (core::df-froundeven :flags (:flushable))
    ((:double-float :double-float) :double-float :double-float)
  (inst)
  (let* ((dividend (in (first (bir:inputs inst))))
         (divisor (in (second (bir:inputs inst))))
         (quotient (%fdiv dividend divisor))
         (iquot (%intrinsic-call "llvm.roundeven.f64" (list quotient)))
         (rem (%fsub dividend (%fmul iquot divisor))))
    (list iquot rem)))
...
(defvprimop (core:check-bound :flags (:flushable))
    ((:utfixnum) :object :utfixnum :object) (inst)
  (let* ((bound (in (second (bir:inputs inst))))
         (index (in (third (bir:inputs inst))))
         (index-fixnump
           (cmp:tag-check-cond index cmp:+fixnum-mask+ cmp:+fixnum00-tag+))
         (checkb (cmp:irc-basic-block-create "check-bound"))
         (successb (cmp:irc-basic-block-create "check-bound-success"))
         (errorb (cmp:irc-basic-block-create "check-bound-fail")))
    (cmp:irc-cond-br index-fixnump checkb errorb)
    (cmp:irc-begin-block errorb)
    (%intrinsic-invoke-if-landing-pad-or-call
     "cc_badIndexError" (list (in (first (bir:inputs inst)))
                              (%size_t 0) index bound))
    (cmp:irc-unreachable)
    (cmp:irc-begin-block checkb)
    (let* ((rindex (cmp:irc-untag-fixnum index cmp:%i64%))
           (posp (cmp:irc-icmp-sge rindex (%i64 0) "index-plusp"))
           (boundp (cmp:irc-icmp-slt rindex bound "index-boundp"))
           (cond (cmp:irc-and posp boundp)))
      (cmp:irc-cond-br cond successb errorb)
      (cmp:irc-begin-block successb)
      rindex)))
...
(defvprimop cons ((:object) :object :object) (inst)
  (cmp:irc-cons (in (first (bir:inputs inst)))
                (in (second (bir:inputs inst)))))
