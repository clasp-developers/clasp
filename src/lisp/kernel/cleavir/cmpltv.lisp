...
(defun add-call-args (name args)
  ...
  (let ((fixed-args (mapcar (lambda (x) (if (core:fixnump x) (cmp:jit-constant-size_t x) x))
                            args)))
    (cmp:irc-create-call name fixed-args)))

(defun add-call (name ltv index &rest args)
  "Call the named function after converting fixnum args to llvm constants"
  (let ((fixed-args (mapcar (lambda (x) (if (core:fixnump x) (cmp:jit-constant-size_t x) x))
                            (list* index args))))
    (cmp:irc-create-call name (list* ltv fixed-args))))
...
(defun ltv/fixnum (fixnum index read-only-p)
  (with-add-init (ltv)
    (add-call "ltvc_make_fixnum" ltv index (cmp:jit-constant-i64 fixnum))))
...
(defun ltv/base-string (str index read-only-p)
  (with-add-init (ltv)
    (let* ((constant (llvm-sys:make-string-global cmp:*the-module* str (core:bformat nil "::str[%s]" str)))
           (ptr (llvm-sys:create-in-bounds-gep
                 cmp:*irbuilder* constant
                 (list (cmp:jit-constant-i32 0) (cmp:jit-constant-i32 0)) "ptr")))
...
(defun object-similarity-table-and-creator (object)
  (cond
    ...
    ((core:fixnump object) (values *fixnum-coalesce* #'ltv/fixnum))
    ...
    ((core:double-float-p object) (values *double-float-coalesce* #'ltv/double-float))
    ...
    ((core:bignump object) (values *bignum-coalesce* #'ltv/bignum))
...
(defmacro with-ltv (&body body)
  `(let ((*table-index* 0)
     ...
     (let ((cmp:*load-time-value-result* (cmp:irc-alloca-tmv cmp:*load-time-initializer-environment*
                                                             :irbuilder cmp:*irbuilder-ltv-function-alloca*))
           (cmp:*load-time-value-holder-global-var*
            #|| spacer ||#(llvm-sys:make-global-variable 
                           cmp:*the-module*
                           cmp:+run-and-load-time-value-holder-global-var-type+
                           nil
       ...
       (with-load-time-value-counters
           (ltv-value-counter
            :prologue-code
            (cmp:with-irbuilder (cmp:*irbuilder-ltv-function-alloca*)
              (cmp:cmp-log "Setting up getOrCreateLoad-TimeValueArray\n")
              (cmp:irc-intrinsic "ltvc_get_or_create_load_time_value_array"
                                 cmp:*load-time-value-holder-global-var*
                                 cmp:*gv-source-namestring*
                                 (cmp:jit-constant-size_t ltv-value-counter))
              (cmp:irc-intrinsic "ltvc_assign_source_file_info_handle"
                                 cmp:*gv-source-namestring*
                                 cmp:*gv-source-debug-namestring*
                                 (cmp:jit-constant-i64 cmp:*source-debug-offset*)
                                 (cmp:jit-constant-i32 (if cmp:*source-debug-use-lineno* 1 0))
                                 cmp:*gv-source-file-info-handle*)))
         (progn
           (load-time-reference-literal nil t)
           (load-time-reference-literal t t))
         ,@body)
       (cmp:with-irbuilder (cmp:*irbuilder-ltv-function-body*)
         (let ((cmp:*gv-current-function-name* (cmp:jit-make-global-string-ptr
                                                (llvm-sys:get-name ,ltv-init-fn) "fn-name")))
           (cmp:with-landing-pad (cmp:irc-get-terminate-landing-pad-block ,fn-env-gs)
             (cmp:irc-function-cleanup-and-return ,fn-env-gs ,result )))))))
...
(defun pretty-load-time-name (object ltv-idx)
  (cond
    ((symbolp object) (core:bformat nil "SYMBOL->%s" object))
    ((consp object) "CONS")
    ((arrayp object) "ARRAY")
    ((numberp object) (format nil "NUMBER->~a" object))
    (t (subseq (core:bformat nil "ltv-idx_%d_val->%s" ltv-idx object) 0 30))))
...
(defun new-run-time-table-index ()
  "Return the next ltv-index. If this is being invoked from COMPILE then
the value is put into *default-load-time-value-vector* and its index is returned"
  (core:data-vector-push-extend cmp:*run-time-values-table* nil 16))

(defun add-run-time-object (object index)
  (core:load-time-value-array-setf cmp:*run-time-values-table* index object))
...
(defun bclasp-compile-form (form)
  ...
  (let ((fn (cmp:with-new-function (fn fn-env fn-result
              ...
              (let* ((given-name (llvm-sys:get-name fn)))
                ...
                (cmp:cmp-log "Creating ltv thunk with name: %s\n" given-name)
                (cmp:dbg-set-current-debug-location-here)
                (cmp:codegen fn-result form fn-env)
                (cmp:dbg-set-current-debug-location-here)))))
    (cmp:cmp-log-dump fn)
    (cmp:irc-verify-function fn t)
    fn))
...
(defun compile-reference-to-literal (literal)
  ...
  (let ((ltv-idx (reference-literal literal t)))
    (unless (core:fixnump ltv-idx) (error "Could not compile-reference-to-literal: ~a" literal))
...
(defun compile-ltv-thunk (form)
  ...
  (let* ((ltv-index (new-table-index))
	 (fn (cmp:with-new-function (fn fn-env fn-result
				    :function-name 'bclasp-top-level-form
				    :parent-env nil
				    :function-form form)
	       (let* ((given-name (llvm-sys:get-name fn)))
		 ...
		 (let ((ltv-result (load-time-value-reference
		   ...
		   (cmp:dbg-set-current-debug-location-here)
		   (cmp:codegen ltv-result form fn-env)
		   (cmp:irc-intrinsic "copyTsp" fn-result ltv-result)
		   (cmp:dbg-set-current-debug-location-here))))))
    (cmp:cmp-log-dump fn)
    (cmp:irc-verify-function fn t)
    (values ltv-index fn)))
...
(defun load-time-value-reference (holder index &optional (label "ltv"))
  #+(or)(let* ((tagged-ltv-ptr (cmp:irc-load holder "tagged-ltv-ptr"))
               (tagged-ltv-intptr_t (cmp:irc-ptr-to-int tagged-ltv-ptr +intptr_t+ "tagged-ltv-intptr_t"))
               (general-pointer-tag (cdr (assoc :general-tag cmp::+cxx-data-structures-info+)))
               (ltvo-address (llvm-sys:create-add cmp:*irbuilder* tagged-ltv-intptr_t (jit-constant-uintptr_t (- general-pointer-tag)) "ltvo_address"))
               (ltvo-objects-offset (cdr (assoc :load-time-values-objects-offset cmp::+cxx-data-structures-info+)))
               (ltvo-objects-address (llvm-sys:create-add cmp:*irbuilder* ltvo-address (jit-constant-uintptr_t ltvo-objects-offset) "ltvo_objects_address"))
               (tagged-ltvo-objects-smart-ptr (cmp:irc-load (cmp:irc-int-to-ptr ltvo-objects-address +tsp*+) "tagged-ltvo-objects-ptr"))
               (tagged-ltvo-objects-ptr (cmp:irc-smart-ptr-extract tagged-ltvo-objects-smart-ptr "tagged-ltvo-objects-ptr"))
               (tagged-ltvo-objects-addr (cmp:irc-ptr-to-int tagged-ltvo-objects-ptr +uintptr_t+ "tagged-ltvo-objects-addr"))
               (ltvo-objects-addr (cmp:irc-add tagged-ltvo-objects-addr (jit-constant-uintptr_t (- general-pointer-tag)) "ltvo-objects-addr"))
               ...
               (entry-uintptr_t (cmp:irc-add ltvo-objects-addr (jit-constant-uintptr_t offset) "entry-uintptr_t"))
               (entry-ptr (cmp:irc-int-to-ptr entry-uintptr_t +tsp*+ (bformat nil "entry[%d]-ptr" index))))
          #+(or)(let ((orig (cmp:irc-intrinsic "loadTimeValueReference" holder (jit-constant-size_t index) label)))
                  (cmp:irc-int-to-ptr (cmp:irc-intrinsic "debug_match_two_uintptr_t"
                                                 (cmp:irc-ptr-to-int entry-ptr +uintptr_t+)
                                                 (cmp:irc-ptr-to-int orig +uintptr_t+))
                                  +tsp*+))
          entry-ptr)
  (cmp:irc-intrinsic "loadTimeValueReference" holder (cmp:jit-constant-size_t index) label))

(defun get-load-time-value (result holder index)
  #+(or)(cmp:irc-intrinsic "getLoadTimeValue"
                 result
                 holder
...
(defun copy-load-time-value (result holder index)
  #+(or)(cmp:irc-intrinsic "copyLoadTimeValue" result holder (jit-constant-size_t index))
