...
(defmacro define-binary-primop (name ircop type)
  `(defmethod translate-primop ((name (eql ',name)) inst)
...
(define-binary-primop core::two-arg-sf-+ %fadd cmp:%float%)
(define-binary-primop core::two-arg-sf-- %fsub cmp:%float%)
(define-binary-primop core::two-arg-sf-* %fmul cmp:%float%)
(define-binary-primop core::two-arg-sf-/ %fdiv cmp:%float%)

(define-binary-primop core::two-arg-df-+ %fadd cmp:%double%)
(define-binary-primop core::two-arg-df-- %fsub cmp:%double%)
(define-binary-primop core::two-arg-df-* %fmul cmp:%double%)
(define-binary-primop core::two-arg-df-/ %fdiv cmp:%double%)

(defmethod translate-primop ((name (eql 'core::sf-expt)) inst)
  (assert (= 2 (length (bir:inputs inst))))
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
    (assert (llvm-sys:type-equal (llvm-sys:get-type i1) cmp:%float%))
    (assert (llvm-sys:type-equal (llvm-sys:get-type i2) cmp:%float%))
    (out (%intrinsic-call "llvm.pow.f32" (list i1 i2))
         (first (bir:outputs inst)))))

(defmethod translate-primop ((name (eql 'core::sf-ftruncate)) inst)
  (assert (= 2 (length (bir:inputs inst))))
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
    (assert (llvm-sys:type-equal (llvm-sys:get-type i1) cmp:%float%))
    ...
    (out (list (%intrinsic-call "llvm.trunc.f32" (list (%fdiv i1 i2)))
               (%frem i1 i2))
         (first (bir:outputs inst)))))
(defmethod translate-primop ((name (eql 'core::df-ftruncate)) inst)
  (assert (= 2 (length (bir:inputs inst))))
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
...
(defmethod translate-primop ((name (eql 'core::df-expt)) inst)
  (assert (= 2 (length (bir:inputs inst))))
  (let ((i1 (in (first (bir:inputs inst))))
        (i2 (in (second (bir:inputs inst)))))
    (assert (llvm-sys:type-equal (llvm-sys:get-type i1) cmp:%double%))
    (assert (llvm-sys:type-equal (llvm-sys:get-type i2) cmp:%double%))
    (out (%intrinsic-call "llvm.pow.f64" (list i1 i2))
         (first (bir:outputs inst)))))

(defmacro define-unary-primop (name intrinsic type)
  `(defmethod translate-primop ((name (eql ',name)) inst)
     (assert (= 1 (length (bir:inputs inst))))
     (let ((arg (in (first (bir:inputs inst)))))
       (assert (llvm-sys:type-equal (llvm-sys:get-type arg) ,type))
       (out (%intrinsic-call ',intrinsic (list arg))
            (first (bir:outputs inst))))))

(define-unary-primop core::sf-abs    "llvm.fabs.f32" cmp:%float%)
(define-unary-primop core::sf-sqrt   "llvm.sqrt.f32" cmp:%float%)
(define-unary-primop core::sf-exp    "llvm.exp.f32"  cmp:%float%)
(define-unary-primop core::sf-log    "llvm.log.f32"  cmp:%float%)
(define-unary-primop core::sf-cos    "llvm.cos.f32"  cmp:%float%)
(define-unary-primop core::sf-sin    "llvm.sin.f32"  cmp:%float%)
(define-unary-primop core::sf-tan    "tanf"          cmp:%float%)
(define-unary-primop core::sf-acos   "acosf"         cmp:%float%)
(define-unary-primop core::sf-asin   "asinf"         cmp:%float%)
(define-unary-primop core::sf-cosh   "coshf"         cmp:%float%)
(define-unary-primop core::sf-sinh   "sinhf"         cmp:%float%)
(define-unary-primop core::sf-tanh   "tanhf"         cmp:%float%)
(define-unary-primop core::sf-acosh  "acoshf"        cmp:%float%)
(define-unary-primop core::sf-asinh  "asinhf"        cmp:%float%)
(define-unary-primop core::sf-atanh  "atanhf"        cmp:%float%)

(define-unary-primop core::df-abs    "llvm.fabs.f32" cmp:%double%)
(define-unary-primop core::df-sqrt   "llvm.sqrt.f32" cmp:%double%)
(define-unary-primop core::df-exp    "llvm.exp.f32"  cmp:%double%)
(define-unary-primop core::df-log    "llvm.log.f32"  cmp:%double%)
(define-unary-primop core::df-cos    "llvm.cos.f32"  cmp:%double%)
(define-unary-primop core::df-sin    "llvm.sin.f32"  cmp:%double%)
(define-unary-primop core::df-tan    "tan"           cmp:%double%)
(define-unary-primop core::df-acos   "acos"          cmp:%double%)
(define-unary-primop core::df-asin   "asin"          cmp:%double%)
(define-unary-primop core::df-cosh   "cosh"          cmp:%double%)
(define-unary-primop core::df-sinh   "sinh"          cmp:%double%)
(define-unary-primop core::df-tanh   "tanh"          cmp:%double%)
(define-unary-primop core::df-acosh  "acosh"         cmp:%double%)
(define-unary-primop core::df-asinh  "asinh"         cmp:%double%)
(define-unary-primop core::df-atanh  "atanh"         cmp:%double%)

(defmethod translate-primop ((name (eql 'core::sf-negate)) inst)
  (assert (= 1 (length (bir:inputs inst))))
  (let ((arg (in (first (bir:inputs inst)))))
    (assert (llvm-sys:type-equal (llvm-sys:get-type arg) cmp:%float%))
    (out (%fneg arg) (first (bir:outputs inst)))))

(defmethod translate-primop ((name (eql 'core::df-negate)) inst)
  (assert (= 1 (length (bir:inputs inst))))
  (let ((arg (in (first (bir:inputs inst)))))
