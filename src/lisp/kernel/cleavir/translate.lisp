...
(defmethod translate-terminator ((instruction bir:catchi) abi next)
  ...
  (let* ((bufp (cmp:alloca cmp::%jmp-buf-tag% 1 "catch-jmp-buf"))
         ...
         (catch (cmp:irc-tag-general (cmp:irc-skip-general-header catch-space)
                                     "catch-dynenv"))
         (tag (in (bir:input instruction))))
    (%intrinsic-call "cc_initialize_catch_dynenv" (list catch-space bufp tag))
    (new-de-stack dcons-space catch)
...
(defmethod translate-terminator ((instruction bir:unwind-protect) abi next)
  ...
  (let* ((cleanup (cmp:irc-basic-block-create "unwind-protect-cleanup"))
         ...
         (old-de-stack (cmp::thread-dynenv-stack))
         (upde (cmp:irc-tag-general (cmp:irc-skip-general-header upde-mem)
                                    "unwind-protect-dynenv"))
         (_ (progn
              (%intrinsic-call "cc_initialize_cleanup_dynenv" (list upde-mem bufp))
              (new-de-stack de-cons-mem upde)))
         (sj (%intrinsic-call "_setjmp" (list bufp))))
    (declare (ignore _))
...
(defmethod translate-terminator ((instruction bir:progvi) abi next)
  ...
  (let* ((inputs (bir:inputs instruction))
         ...
         (old-de-stack (cmp::thread-dynenv-stack)))
    (%intrinsic-call "cc_initialize_progv_dynenv" (list pde-mem cells oldvals))
    (new-de-stack pde-cons-mem
                  (cmp:irc-tag-general (cmp:irc-skip-general-header pde-mem "progv-dynenv")
                                       "progv-dynenv"))
