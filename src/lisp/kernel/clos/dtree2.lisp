(defpackage #:test-clos (:use #:cl))

(in-package #:test-clos)
...
(defun specializer= (s1 s2)
  (if (clos::eql-specializer-p s1)
      (and (clos::eql-specializer-p s2)
           (eql (clos::eql-specializer-value s1) (clos::eql-specializer-value s2)))
      ;; for classes:
      (eq s1 s2)))
...
(defun differentiate-specializers (paths)
  (loop with eqls = nil
        ...
        for pair in paths
        for spec = (car pair)
        do (cond ((clos::eql-specializer-p spec) (push pair eqls))
                 ((tag-spec-p spec)
                  (setf (svref tags-vector (class-tag spec)) (cdr pair)))
                 ((< (core:class-stamp-for-instances spec) cmp:+c++-stamp-max+)
                  (setf c++-classes
                        (clos::insert-sorted pair c++-classes #'< #'path-pair-key)))
                 (t
                  (setf other-classes
                        (clos::insert-sorted pair other-classes #'< #'path-pair-key))))
...
(defun next= (next1 next2)
  (if (clos::outcome-p next1)
      (and (clos::outcome-p next2) (clos::outcome= next1 next2))
      (eq next1 next2)))
...
(defun linearize (tree)
  (cond
    ((clos::outcome-p tree) (linearize-outcome tree))
...
(defun linearize-outcome (outcome)
  (cond ((clos::optimized-slot-reader-p outcome)
         (list 'clos::optimized-slot-reader
               (clos::optimized-slot-reader-index outcome)
               (clos::optimized-slot-reader-slot-name outcome)
               (clos::optimized-slot-reader-class outcome)))
        ((clos::optimized-slot-writer-p outcome)
         (list 'clos::optimized-slot-writer
               (clos::optimized-slot-writer-index outcome)))
        ((clos::fast-method-call-p outcome)
         (list 'clos::fast-method-call
               (clos::fast-method-call-function outcome)))
        ((clos::effective-method-outcome-p outcome)
         (list 'clos::effective-method-outcome
               (clos::effective-method-outcome-function outcome)))))
...
(defvar *isa*
  '((miss 0) (advance 1) (tag-test 2) (header-stamp-read 3) (where-branch 4)
    ...
    (clos::optimized-slot-reader 10) (clos::optimized-slot-writer 11)
    (clos::fast-method-call 12) (clos::effective-method-outcome 13)))
