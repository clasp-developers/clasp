       ...
       (let ((total (or total-files (length files)))
         ...
         (labels ((entry-position (entry)
                  ...
                  (report-result-stream (result-stream)
                    (mmsg "In report-result-stream: %s%N" result-stream)
                    (let ((buffer (get-output-stream-string result-stream)))
                      (let ((sin (make-string-input-stream buffer)))
             ...
             (tagbody
              top
                ...
                (when (> job-counter parallel-jobs)
                  ...
                  (multiple-value-setq (wpid status) (wait-for-child-to-exit jobs))
                  (mmsg "Exited from wait-for-child-to-exit%N")
                  (if (>= wpid 0)
                      (let* ((finished-entries-triplet (gethash wpid jobs))
                             (finished-entries (pjob-entries finished-entries-triplet))
                             (result-stream (pjob-result-stream finished-entries-triplet)))
                        (setf-pjob-done finished-entries-triplet t)
