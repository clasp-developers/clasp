	...
	(let* ((irbuilder-alloca (llvm-sys:make-irbuilder *llvm-context*))
	  ...
	  (let ((body-bb (irc-basic-block-create "body" disp-fn))
	    ...
	    (with-irbuilder (irbuilder-alloca)
	      (let* ((in-frame-va_list             (irc-alloca-va_list :label "in-frame-va_list"))
                     (gf-args-uint                 (irc-ptr-to-int gf-args %uintptr_t% "in-frame-va_list-uint"))
                     (gf-args-va_list-uint         (irc-add gf-args-uint (jit-constant-intptr_t (- +VaList_S-valist-offset+ +VaList_S-tag+)) "gf-args-va_list-uint"))
                     (gf-args-va_list              (irc-int-to-ptr gf-args-va_list-uint %va_list*% "gf-args-va_list"))
                     (_                            (insert-message))
                     (_                            (debug-arglist (irc-ptr-to-int gf-args-va_list %uintptr_t%)))
                     (_                            (irc-intrinsic-call "llvm.va_copy" (list (irc-pointer-cast in-frame-va_list %i8*% "in-frame-va_list-i8*")
                                                                                         (irc-pointer-cast gf-args-va_list %i8*% "gf-args-va_list-i8*"))))
                     (_                            (insert-message))
                     (debug-arglist                (irc-ptr-to-int in-frame-va_list %uintptr_t%))
                     (_                            (irc-br body-bb)))
		(with-irbuilder (irbuilder-body)
		  (codegen-node-or-outcome (dtree-root dtree) in-frame-va_list gf gf-args))
		(irc-begin-block *bad-tag-bb*)
		(irc-intrinsic-call "llvm.va_end" (list (irc-pointer-cast in-frame-va_list %i8*% "in-frame-va_list-i8*")))
