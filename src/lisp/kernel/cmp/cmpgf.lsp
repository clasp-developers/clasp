...
(progn
  (defun insert-message ()
    (irc-create-call "debugPrintI32" (list (jit-constant-i32 (incf *message-counter*)))))
...
(defun codegen-arg-stamp (arg gf gf-args)
  ...
  ;; First check the tag
  (insert-message)
  (let ((tag (irc-and (irc-ptr-to-int arg +uintptr_t+) (jit-constant-uintptr_t +tag-mask+)))
	(fixnum-bb (irc-basic-block-create "fixnum-bb"))
...
(defun codegen-class-specializers (node arg args gf gf-args)
  (let ((arg-stamp (codegen-arg-stamp arg gf gf-args)))
    (debug-call "debugPrint_size_t" (list arg-stamp))
