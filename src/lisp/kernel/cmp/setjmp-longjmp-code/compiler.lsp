...
(defun parse-macro (name vl body &optional env)
  (multiple-value-bind (lambdablock ppn doc)
...
(defun codegen-global-function-call (result sym env)
  ...
  (irc-call env "invokePossibleMultipleValueSymbolFunction" result (irc-global-symbol sym env) (irc-renv env)))
...
Return the new environment."
  (irc-new-value-environment
   ...
   :label "copy-req-args"
   :fill-runtime-form (lambda (new-env) (irc-call new-env "copyAFsp" (irc-renv new-env) activation-frame))))
...
(defun compile-copy-activation-frame (old-env new-env lambda-list-handler src-activation-frame dest-activation-frame)
  (let ((number-of-required-arguments (number-of-required-arguments lambda-list-handler)))
    ...
    (dolist (classified-local (classified-symbols lambda-list-handler))
      (cond
	((eq (car classified-local) 'lexical-var)
	...
	((eq (car classified-local) 'special-var)
    ...
    (irc-call new-env "copyAFsp" dest-activation-frame src-activation-frame)
    ))
...
;; TODO: Should I attach debugging info to a value frame that I'm copying?
;;	(irc-attach-debugging-info-to-value-frame (irc-renv new-env) lambda-list-handler new-env)
      ...
      (let ((closed-renv (irc-call new-env "activationFrameParentRef" src-activation-frame))
	    (lexical-names (names-of-all-lexical-variables lambda-list-handler)))
...
;;;	(irc-use-existing-value-frame new-env (irc-renv new-env) (number-of-lexical-variables lambda-list-handler))
	(irc-call new-env "setParentOfActivationFrame" (irc-renv new-env) closed-renv)
	(compile-lambda-list-code lambda-list-handler old-env src-activation-frame new-env)
	(dbg-set-current-debug-location-here)
	(irc-attach-debugging-info-to-value-frame (irc-renv new-env) lambda-list-handler new-env)
;;; old-version (irc-attach-debugging-info-to-value-frame (irc-renv new-env) lexical-names new-env)	
	)))
...
- otherwise return the cons of llvm-sys::Function_sp's that were compiled for the lambda"
  (assert-result-isa-llvm-sys-value result)
  (let* ((*all-funcs-for-one-compile* nil)
	 (compiled-fn (compile-lambda-function lambda env :function-name-as-string function-name)))
    (if result
	(let ((funcs (compile-reference-to-literal (if *generate-compile-file-load-time-values*
						       nil
						       *all-funcs-for-one-compile*) env)))
	  (irc-call env "makeCompiledFunction" result compiled-fn funcs (irc-renv env)))
...
(defun codegen-global-function-lookup (result sym env)
  (irc-call env "symbolFunctionRead" result (irc-global-symbol sym env)))


(defun codegen-global-setf-function-lookup (result setf-function-name env)
  (let ((setf-symbol (cadr setf-function-name)))
    (irc-call env "setfSymbolFunctionRead" result (irc-global-setf-symbol setf-symbol env))
  ))
...
env is the parent environment of the result-af value frame"
  (dbg-set-current-debug-location-here)
  (irc-use-existing-value-frame parent-env result-af (length exps))
;;  (irc-call parent-env "makeValueFrame" result-af (jit-constant-i32 (length exps)))
  (irc-call parent-env "setParentOfActivationFrame" result-af (irc-renv parent-env))
...
(defun codegen-multiple-value-call (result rest env)
  (with-dbg-lexical-block (env rest)
    (let ((accumulate-results (irc-alloca-tsp env :label "acc-multiple-value-results"))
	  ...
	  (func (irc-alloca-tsp env :label "func"))
	...
	(irc-call env "prependMultipleValues" accumulate-results temp-mv-result)
	)
      (irc-call env "makeValueFrameFromReversedCons" accumulated-af accumulate-results)
      (irc-call env "setParentOfActivationFrame" accumulated-af (irc-renv env))
      
      (codegen func (car rest) env)
      (irc-call env "invokePossibleMultipleValueFunction" temp-mv-result func accumulated-af)
      (irc-call env "copyTmvOrSlice" result temp-mv-result)
      )
    ))
...
(defun codegen-multiple-value-prog1 (result rest env)
  (with-dbg-lexical-block (env rest)
    (let ((temp-mv-result (irc-alloca-tmv env :label "temp-mv-result"))
	  (saved-values (irc-alloca-tsp env :label "saved-values"))
	  (temp-val (irc-alloca-tsp env :label "temp-val")))
      ;; See the interpreter sp_multipleValueCall
      (codegen temp-mv-result (car rest) env)
      (irc-call env "saveValues" saved-values temp-mv-result)
      (do* ((cur (cdr rest) (cdr cur))
	    (form (car cur) (car cur)))
	   ((endp cur) nil)
	(codegen temp-val form env)
	)
      (irc-call env "loadValues" temp-mv-result saved-values)
      (irc-call env "copyTmvOrSlice" result temp-mv-result)
      )))
...
(defun codegen-special-var-reference (var &optional env)
  (irc-call env "symbolValueReference" (irc-global-symbol var env) (bformat nil "<special-var:%s>" var )))
  ...
  (if setq-pairs
	...
	(let ((expanded (macroexpand cur-var env)))
	  (if (eq expanded cur-var)
		...
		(codegen result `(setf ,expanded ,cur-expr) env))
	      )))
...
(defun codegen-fill-let/let*-environment-premade-value-frame ( new-env lambda-list-handler exps parent-env evaluate-env)
  "Evaluate each of the exps in the evaluate-env environment
...
env is the parent environment of the (result-af) value frame"
  (multiple-value-bind (reqvars)
    ...
    (let ((number-of-lexical-vars (number-of-lexical-variables lambda-list-handler))
      ...
      (dbg-set-current-debug-location-here)
      (irc-use-existing-value-frame parent-env result-af number-of-lexical-vars)
      ;;      (irc-call parent-env "makeValueFrame" result-af (jit-constant-i32 number-of-lexical-vars))
      (irc-call parent-env "setParentOfActivationFrame" result-af (irc-renv parent-env))
      (dbg-set-current-debug-location-here)
...
;;      (break "make sure that divergence with kaleidoscope demo works")
      ...
      (let* ((thefunction *current-function*)
...
jump to blocks within this tagbody."
  ...
  (let ((tagbody-env (make-tagbody-environment env)))
    (let ((enumerated-tag-blocks (tagbody.enumerate-tag-blocks rest tagbody-env)))
      (with-setjmp-longjmp (tagbody-env jmp-buf-var)
	(progn
	  (set-longjmp-info tagbody-env jmp-buf-var)
	  (mapl #'(lambda (cur)
	...
	(let* ((go-index (irc-call env "setjmp_user0_get_i32" jmp-buf-var))
...
(defun codegen-go (result rest env)
  (dbg-set-current-debug-location-here)
  (let* ((tag (car rest))
	 (tagbody-env (find-tagbody-tag-environment env tag))
	 (classified-tag (classify-tag env tag)))
    (if tagbody-env
	(let ((longjmp-info (get-longjmp-info tagbody-env))
	      (index (cdr classified-tag)))
	  (irc-setjmp.buf-user0-set-i32 tagbody-env longjmp-info index)
	  (irc-low-level-trace)
	  (irc-call env "llvm.eh.sjlj.longjmp" (llvm-sys:create-bit-cast *irbuilder* longjmp-info +i8*+ "setjmp-buf")))
...
(defun codegen-return-from (result rest env)
  ...
  (let* ((temp-mv-result (irc-alloca-tmv env :label "temp-mv-result"))
	...
	(let ((block-depth (calculate-block-depth env block-symbol)))
	  ...
	  (irc-low-level-trace)
	  (irc-call env "throwReturnFrom" (jit-constant-i32 block-depth) temp-mv-result))
	...
	(let ((evaluate-env (cond
	  ...
	  (with-try evaluate-env
	    (progn
	      ...
	      (irc-call function-env "trace_setActivationFrameForLexicalScope" traceid (irc-renv function-env))
...
(defun codegen-the/truly-the (result rest env)
...
(defun codegen-locally (result rest env)
  (with-dbg-lexical-block (env rest)
	...
	(irc-use-existing-value-frame env (irc-renv new-env) 0)
	;;      (irc-call env "makeValueFrame" (irc-renv new-env) (jit-constant-i32 0))
	(irc-call env "setParentOfActivationFrame" (irc-renv new-env) (irc-renv env))
	(dolist (sp specials)
...
(defun codegen-throw (result rest env)
  (let ((tag (car rest))
    ...
    (let ((tag-store (irc-alloca-tsp env :label "tag-store"))
      ...
      (irc-low-level-trace)
      (irc-call env "throwCatchThrow" tag-store result-mv-form-store))))
...
(defun codegen-special-operator (result head rest env)
  ...
  (cond
    ...
    ((eq head 'the) (codegen-the/truly-the result rest env ))
    ((eq head 'truly-the) (codegen-the/truly-the result rest env))
  ...
  (let ((code (get-code func))
    ...
    (let* ((fn (compile-lambda/lambda-block (symbol-name name)
	   ...
	   (compiled-function (llvm-sys:get-compiled-function *run-time-execution-engine*
							      ...
							      (function-kind func))))
      (core::setf-symbol-function name compiled-function)
      compiled-function)))
  ...
  (cond
    ...
    ((consp definition)
     ...
     (let* ((fn (compile-lambda-function definition
					...
					:function-name-as-string (if name
							   (symbol-name name)
							   "lambda")))
	    (compiled-function (llvm-sys:get-compiled-function *run-time-execution-engine* name fn
							       (environment-activation-frame env) :function))
	    )
       (when name
    ...
    ((null definition)
	 ...
	 ((interpreted-function-p func)
	  (values (compile-interpreted-function name func) nil nil)))))
...
(defun compile-in-env (name &optional definition env)
  ...
  (with-compiler-env ()
    (let ((*the-module* *run-time-module*)
	  (*the-function-pass-manager* *run-time-function-pass-manager*))
      (declare (special *the-function-pass-manager*))
      (with-irbuilder (env (llvm-sys:make-irbuilder *llvm-context*))
	(let ((*gv-source-path-name* (jit-make-global-string-ptr *source-path-name* "source-path-name"))
	      (*all-funcs-for-one-compile* nil))
	  (multiple-value-bind (compiled-function warnp failp)
	      (compile* name definition env)
	    ;;      (break "check compiled-function")
	    (let* ((compiled-body (get-body compiled-function)))
...
(defun disassemble (desig)
  (cond
    ...
    ((and (consp desig) (or (eq (car desig) 'lambda) (eq (car desig) 'lambda-block)))
