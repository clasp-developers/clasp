...
(defun compile-header-check (header-value-min-max object-raw then-br else-br)
  (let ((header-check-br  (irc-basic-block-create "header-check-br")))
    ...
    (let* ((byte-ptr           (irc-bit-cast object-raw %i8*%))
           (header-addr        (irc-gep byte-ptr (list (jit-constant-i64 (+ +header-stamp-offset+ (- (+ +header-size+ +general-tag+)))))))
           ...
           (header-value       (irc-bit-cast (irc-load (irc-bit-cast header-addr header-stamp-type)) %i64%)))
