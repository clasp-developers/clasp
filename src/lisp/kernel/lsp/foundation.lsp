...
(defun hash-table-pairs-maphash (hashtable)
  (let ((result (make-array (* 2 (hash-table-count hashtable))))
        (index 0))
    (maphash #'(lambda(key value)
                 (setf (aref result index) key)
                 (incf index)
...
(defun hash-table-iterator (hash-table)
  (let ((pairs (if (typep hash-table 'CORE:WEAK-KEY-HASH-TABLE)
                   (hash-table-pairs-maphash hash-table)
                   (core:hash-table-pairs hash-table)))
        (hash-index 0))
    (function (lambda ()
