   ...
   (with-start-end (start2 end2 sequence2)
     ...
     (let ((length (min (- end2 start2) (- end1 start1))))
       ...
       (if (and (vectorp sequence1)
           ...
           (let ((data (if (and (eq sequence1 sequence2)
                                (> start1 start2))
                           (subseq sequence2 start2 end2)
                           sequence2)))
             (multiple-value-bind (it1 ref1 set1 next1)
                 (make-seq-iterator sequence1 start1)
               (declare (ignore ref1))
               (multiple-value-bind (it2 ref2 set2 next2)
                   (make-seq-iterator data start2)
                 (declare (ignore set2))
                 (do ((it1 it1 (funcall next1 it1))
                      (it2 it2 (funcall next2 it2)))
                     ;; NULL = seq-iterator-endp
                     ((or (<= length 0) (null it1) (null it2)))
                   (funcall set1 sequence1 it1
                            (funcall ref2 sequence2 it2))
                   (decf length)))))))))
  sequence1)
    ...
    (with-start-end (start1 end1 sequence1)
      (with-start-end (start2 end2 sequence2)
        (multiple-value-bind (it1 ref1 set1 next1)
            (make-seq-iterator sequence1 start1)
          (multiple-value-bind (it2 ref2 set2 next2)
              (make-seq-iterator sequence2 start2)
            (do* ((last-index -1)
                  (start2 start2 (1+ start2))
                  (last (let* ((l (- end1 start1))
                               (e (- end2 l)))
                          ...
                          (unless (plusp l)
                            (return-from search-generic
                              (if from-end end2 0)))
                          (1+ e)))
                  (base (key (funcall ref1 sequence1 it1)))
                  (it2 it2 (funcall next2 it2)))
                 ;; NULL = seq-iterator-endp
                 ((or (null it2) (>= start2 last))
                  (if (minusp last-index) nil last-index))
              (declare (fixnum start2 last-index last))
              (when (compare base (key (funcall ref2 sequence2 it2)))
                (let* ((it1 it1)
                       (it2 it2)
                       (i1 start1)
                       (i2 start2))
                  (declare (fixnum i1 i2))
                  (loop
                    (setf it1 (funcall next1 sequence1 it1)
                          it2 (funcall next2 sequence2 it2)
                          i1 (1+ i1)
                          i2 (1+ i2))
                    (when (or (>= i1 end1) (>= i2 end2)
                              (null it1) (null it2))
                    ...
                    (unless (compare (key (funcall ref1 sequence1 it1))
                                     (key (funcall ref2 sequence2 it2)))
                      (return))))))))))))
