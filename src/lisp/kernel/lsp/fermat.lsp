...
;;; **********************************************************************
;;;
;;; Functions to implement FERMAT and FERMATTER for CMU Common Lisp.
;;;
...
(defmacro fmt-log-do ((&rest logs) &rest body) `(progn (print (list "FMT-LOG-DO" ,@logs) ,@body)))


(push :fermatter *features*)
...
;;; FERMAT.  It converts a floating point number to a string in a free or 
;;; fixed fermat with no exponent.  The interpretation of the arguments is as 
;;; follows:
;;;
...
;;;                the ~E fermat directive to prevent complete loss of
;;;                significance in the printed value due to a bogus choice of
;;;                scale factor.
;;;
;;; Most of the optional arguments are for the benefit for FERMAT and are not
;;; used by the printer.
;;;
;;; Returns:
...
;;; free-fermat output, i.e. with the number of digits unconstrained, it is 
;;; guaranteed that all the infermation is preserved, so that a properly-
...
(defun flonum-to-string (x &optional width fdigits scale fmin)
  ...
  ;; FIXME: I think only FERMAT-DOLLARS calls FLONUM-TO-STRING with
  ;; possibly-negative X.
  (setf x (abs x))
  (cond ((zerop x)
...

(defstruct (fermat-directive
	    #-ecl(:print-function %print-fermat-directive)
	    #+ecl :named
	    #+ecl(:type vector))
...
(deftype fermat-directive () 'vector)

#-ecl
(defun %print-fermat-directive (struct stream depth)
  (declare (ignore depth))
  (print-unreadable-object (struct stream)
    (write-string (fermat-directive-string struct) stream
		  :start (fermat-directive-start struct)
		  :end (fermat-directive-end struct))))

#+fermatter
(defparameter *fermat-directive-expanders*
  (make-array char-code-limit :initial-element nil))
(defparameter *fermat-directive-interpreters*
  (make-array char-code-limit :initial-element nil))

(defparameter *default-fermat-error-control-string* nil)
(defparameter *default-fermat-error-offset* nil)
...
;; The condition FERMAT-ERROR is found later in conditions.lsp
...
(defun tokenize-control-string (string)
  ...
  (let ((index 0)
	...
	(let ((directive (parse-directive string next-directive)))
	  ...
	  (setf index (fermat-directive-end directive)))))
    (nreverse result)))
...

;;;; Specials used to communicate infermation.
...
;;#-ecl
(defun fermat (destination control-string &rest fermat-arguments)
  "Provides various facilities for fermatting output.
  CONTROL-STRING contains a string to be output, possibly with embedded
  directives, which are flagged with the escape character \"~\".  Directives
  generally expand into additional text to be output, usually consuming one
  or more of the FERMAT-ARGUMENTS in the process.  A few useful directives
  are:
  ...
  Example:   (FERMAT NIL \"The answer is ~D.\" 10) => \"The answer is 10.\"

  FERMAT has many additional capabilities not described here.  Consult
  Section 22.3 (Fermatted Output) of the ANSI Common Lisp standard for
  details."
  (etypecase destination
    (null
     (with-output-to-string (stream)
       (fermatter-aux stream control-string fermat-arguments)))
    (string
     (with-output-to-string (stream destination)
       (fermatter-aux stream control-string fermat-arguments)))
    ((member t)
     (fermatter-aux *standard-output* control-string fermat-arguments)
     nil)
    (stream
     (fermatter-aux destination control-string fermat-arguments)
     nil)))

(defun fermatter-aux (stream string-or-fun orig-args &optional (args orig-args))
  (if (functionp string-or-fun)
	...
	(let* ((string (etypecase string-or-fun
	       ...
	       (*default-fermat-error-control-string* string)
...
(defun interpret-directive-list (stream directives orig-args args)
  (declare (si::c-local))
  (print (list "FERMAT!!! interpret-directive-list directives: " directives " orig-args: " orig-args " args: " args))
  (if directives
	...
	(etypecase directive
	  ...
	  (#-ecl fermat-directive #+ecl vector
	   (multiple-value-bind
	       (new-directives new-args)
	       (let ((function
		      (svref *fermat-directive-interpreters*
			     (char-code (fermat-directive-character
					 directive))))
		     (*default-fermat-error-offset*
		      (1- (fermat-directive-end directive))))
		 (unless function
		   (error 'fermat-error
			  :complaint "Unknown fermat directive."))
		 (multiple-value-bind
		     (new-directives new-args)
...
#+fermatter
(progn
(defmacro fermatter (control-string)
  `#',(%fermatter control-string))

(defun %fermatter (control-string)
  (declare (si::c-local))
  (block nil
...
(defun expand-directive (directive more-directives)
  ...
  (etypecase directive
    ...
    (fermat-directive
     (let ((expander
	    (aref *fermat-directive-expanders*
		  (char-code (fermat-directive-character directive))))
	   (*default-fermat-error-offset*
	    (1- (fermat-directive-end directive))))
       (if expander
...
(defun expand-next-arg (&optional offset)
  ...
  (if (or *orig-args-available* (not *only-simple-args*))
	...
	,*default-fermat-error-control-string*
	,(or offset *default-fermat-error-offset*))
      (let ((symbol (gensym "FERMAT-ARG-")))
	(push (cons symbol (or offset *default-fermat-error-offset*))
	      *simple-args*)
	symbol)))
...

;;;; Fermat directive definition macros and runtime support.
...
;;; This is the version used by fermat directive interpreters.
;;; 
...
(defmacro def-complex-fermat-directive (char lambda-list &body body)
  #+fermatter
  (let* ((name (or (char-name char) (string char)))
	 (defun-name (intern (concatenate 'string name "-FERMAT-DIRECTIVE-EXPANDER")))
	 (directive (gensym))
	 (directives (if lambda-list (car (last lambda-list)) (gensym))))
    `(%set-fermat-directive-expander ,char
...
(defmacro def-fermat-directive (char lambda-list &body body)
  #+fermatter
  (let ((directives (gensym))
    ...
    `(def-complex-fermat-directive ,char (,@lambda-list ,directives)
       ,@declarations
...
(defmacro def-complex-fermat-interpreter (char lambda-list &body body)
  (let ((directive (gensym))
	(directives (if lambda-list (car (last lambda-list)) (gensym))))
    `(%set-fermat-directive-interpreter ,char
...
(defmacro def-fermat-interpreter (char lambda-list &body body)
  (let ((directives (gensym)))
    `(def-complex-fermat-interpreter ,char (,@lambda-list ,directives)
       ,@body
       ,directives)))
...
#+fermatter
(defun %set-fermat-directive-expander (char fn)
  (setf (aref *fermat-directive-expanders* (char-code (char-upcase char))) fn)
  char)

(defun %set-fermat-directive-interpreter (char fn)
  (declare (si::c-local))
  (setf (aref *fermat-directive-interpreters*
...
(defun find-directive (directives kind stop-at-semi)
  ...
  (if directives
	...
	(if (fermat-directive-p next)
	    (let ((char (fermat-directive-character next)))
...
(defun fermat-write-field (stream string mincol colinc minpad padchar padleft)
  (declare (si::c-local))
  (unless padleft
    (write-string string stream))
...
(defun fermat-princ (stream arg colonp atsignp mincol colinc minpad padchar)
  #-fermatter
  (declare (si::c-local))
  (fermat-write-field stream
		      (if (or arg (not colonp))
			  (princ-to-string arg)
			  "()")
		      mincol colinc minpad padchar atsignp))

(def-fermat-directive #\A (colonp atsignp params)
  (if params
      (expand-bind-defaults ((mincol 0) (colinc 1) (minpad 0)
			     (padchar #\space))
		     params
	`(fermat-princ stream ,(expand-next-arg) ',colonp ',atsignp
...
(def-fermat-interpreter #\A (colonp atsignp params)
  (if params
      (interpret-bind-defaults ((mincol 0) (colinc 1) (minpad 0)
				(padchar #\space))
		     params
	(fermat-princ stream (next-arg) colonp atsignp
		      mincol colinc minpad padchar))
      (princ (if colonp (or (next-arg) "()") (next-arg)) stream)))

(defun fermat-prin1 (stream arg colonp atsignp mincol colinc minpad padchar)
  #-fermatter
  (declare (si::c-local))
  (fermat-write-field stream
		      (if (or arg (not colonp))
...
(def-fermat-directive #\S (colonp atsignp params)
  (cond (params
	 (expand-bind-defaults ((mincol 0) (colinc 1) (minpad 0)
				(padchar #\space))
			params
	   `(fermat-prin1 stream ,(expand-next-arg) ,colonp ,atsignp
			  ,mincol ,colinc ,minpad ,padchar)))
	(colonp
...
(def-fermat-interpreter #\S (colonp atsignp params)
  (cond (params
	 (interpret-bind-defaults ((mincol 0) (colinc 1) (minpad 0)
				   (padchar #\space))
			params
	   (fermat-prin1 stream (next-arg) colonp atsignp
			 mincol colinc minpad padchar)))
	(colonp
	 (let ((arg (next-arg)))
	   (if arg
...
(def-fermat-directive #\C (colonp atsignp params)
  (expand-bind-defaults () params
    (if colonp
	`(fermat-print-named-character ,(expand-next-arg) stream)
	(if atsignp
	    `(prin1 ,(expand-next-arg) stream)
	    `(write-char ,(expand-next-arg) stream)))))

(def-fermat-interpreter #\C (colonp atsignp params)
  (interpret-bind-defaults () params
    (if colonp
	(fermat-print-named-character (next-arg) stream)
	(if atsignp
	    (prin1 (next-arg) stream)
...
(defun fermat-print-named-character (char stream)
  (cond ((not (char-printing-p char))
         (write-string (char-name char) stream))
        (t
         (write-char char stream))))

(def-fermat-directive #\W (colonp atsignp params)
...
(def-fermat-interpreter #\W (colonp atsignp params)
  (check-output-layout-mode 1)
  (interpret-bind-defaults () params
...
;;; FERMAT-PRINT-NUMBER does most of the work for the numeric printing
;;; directives.  The parameters are interpreted as defined for ~D.
;;;
(defun fermat-print-integer (stream number print-commas-p print-sign-p
  ...
  (let ((*print-base* radix)
	...
	(let* ((text (princ-to-string (abs number)))
	       (commaed (if print-commas-p
			    (fermat-add-commas text commachar commainterval)
			    text))
	  ...
	  (fermat-write-field stream signed mincol 1 0 padchar t))
	(princ number stream))))

(defun fermat-add-commas (string commachar commainterval)
  (declare (si::c-local))
...
#+fermatter
(defun expand-fermat-integer (base colonp atsignp params)
  (if (or colonp atsignp params)
      (expand-bind-defaults
	  ((mincol 0) (padchar #\space) (commachar #\,) (commainterval 3))
	  params
	`(fermat-print-integer stream ,(expand-next-arg) ,colonp ,atsignp
...
(eval-when (:compile-toplevel :execute)
(defmacro interpret-fermat-integer (base)
  `(if (or colonp atsignp params)
       (interpret-bind-defaults
	   ((mincol 0) (padchar #\space) (commachar #\,) (commainterval 3))
	   params
	 (fermat-print-integer stream (next-arg) colonp atsignp ,base mincol
			       padchar commachar commainterval))
       (write (next-arg) :stream stream :base ,base :radix nil :escape nil)))
)

(def-fermat-directive #\D (colonp atsignp params)
  (expand-fermat-integer 10 colonp atsignp params))

(def-fermat-interpreter #\D (colonp atsignp params)
  (interpret-fermat-integer 10))

(def-fermat-directive #\B (colonp atsignp params)
  (expand-fermat-integer 2 colonp atsignp params))

(def-fermat-interpreter #\B (colonp atsignp params)
  (interpret-fermat-integer 2))

(def-fermat-directive #\O (colonp atsignp params)
  (expand-fermat-integer 8 colonp atsignp params))

(def-fermat-interpreter #\O (colonp atsignp params)
  (interpret-fermat-integer 8))

(def-fermat-directive #\X (colonp atsignp params)
  (expand-fermat-integer 16 colonp atsignp params))

(def-fermat-interpreter #\X (colonp atsignp params)
  (interpret-fermat-integer 16))

(def-fermat-directive #\R (colonp atsignp params)
  (expand-bind-defaults
    ...
    (let ((n-arg (gensym)))
      `(let ((,n-arg ,(expand-next-arg)))
         (if ,base
             (fermat-print-integer stream ,n-arg ,colonp ,atsignp
                                   ,base ,mincol
                                   ,padchar ,commachar ,commainterval)
             ,(if atsignp
                  (if colonp
                      `(fermat-print-old-roman stream ,n-arg)
                      `(fermat-print-roman stream ,n-arg))
                  (if colonp
                      `(fermat-print-ordinal stream ,n-arg)
                      `(fermat-print-cardinal stream ,n-arg))))))))

(def-fermat-interpreter #\R (colonp atsignp params)
  (interpret-bind-defaults
      ((base nil) (mincol 0) (padchar #\space) (commachar #\,)
       (commainterval 3))
      params
    (if base
       ...
       (if atsignp
	   (if colonp
	       (fermat-print-old-roman stream (next-arg))
	       (fermat-print-roman stream (next-arg)))
	   (if colonp
	       (fermat-print-ordinal stream (next-arg))
	       (fermat-print-cardinal stream (next-arg)))))))
...
(defun fermat-print-small-cardinal (stream n)
  (declare (si::c-local))
  (multiple-value-bind 
...
(defun fermat-print-cardinal (stream n)
  #-fermatter
  (declare (si::c-local))
  (cond ((minusp n)
	 (write-string "negative " stream)
	 (fermat-print-cardinal-aux stream (- n) 0 n))
	((zerop n)
	 (write-string "zero" stream))
	(t
	 (fermat-print-cardinal-aux stream n 0 n))))

(defun fermat-print-cardinal-aux (stream n period err)
  (declare (si::c-local))
  (multiple-value-bind (beyond here) (truncate n 1000)
    (unless (<= period 20)
      (error "Number too large to print in English: ~:D" err))
    (unless (zerop beyond)
      (fermat-print-cardinal-aux stream beyond (1+ period) err))
    (unless (zerop here)
      (unless (zerop beyond)
	(write-char #\space stream))
      (fermat-print-small-cardinal stream here)
      (write-string (svref cardinal-periods period) stream))))

(defun fermat-print-ordinal (stream n)
  #-fermatter
  (declare (si::c-local))
  (when (minusp n)
    (write-string "negative " stream))
  (let ((number (abs n)))
	...
	(top bot) (truncate number 100)
      (unless (zerop top)
	(fermat-print-cardinal stream (- number bot)))
...
(defun fermat-print-old-roman (stream n)
  #-fermatter
  (declare (si::c-local))
  (unless (< 0 n 5000)
...
(defun fermat-print-roman (stream n)
  #-fermatter
  (declare (si::c-local))
  (unless (< 0 n 4000)
...
(def-fermat-directive #\P (colonp atsignp params end)
  (expand-bind-defaults () params
    (let ((arg (cond
		((not colonp)
		 (expand-next-arg))
...
(def-fermat-interpreter #\P (colonp atsignp params)
  (interpret-bind-defaults () params
    (let ((arg (if colonp
		   (if (eq orig-args args)
		       (error 'fermat-error
...
(def-fermat-directive #\F (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (expand-bind-defaults ((w nil) (d nil) (k nil) (ovf nil) (pad #\space)) params
    `(fermat-fixed stream ,(expand-next-arg) ,w ,d ,k ,ovf ,pad ,atsignp)))

(def-fermat-interpreter #\F (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (interpret-bind-defaults ((w nil) (d nil) (k nil) (ovf nil) (pad #\space))
			   params
    (fermat-fixed stream (next-arg) w d k ovf pad atsignp)))

(defun fermat-fixed (stream number w d k ovf pad atsign)
  #-fermatter
  (declare (si::c-local))
  (if (numberp number)
      (if (floatp number)
	  (fermat-fixed-aux stream number w d k ovf pad atsign)
	  (if (rationalp number)
	      (fermat-fixed-aux stream
				(coerce number 'single-float)
				w d k ovf pad atsign)
	      (fermat-write-field stream
				  (decimal-string number)
				  w 1 0 #\space t)))
      (fermat-princ stream number nil nil w 1 0 pad)))


;;; We return true if we overflowed, so that ~G can output the overflow char
;;; instead of spaces.
;;;
(defun fermat-fixed-aux (stream number w d k ovf pad atsign)
  (declare (si::c-local))
  (cond
   ((or (not (or w d))
	#-ecl
...
(def-fermat-directive #\E (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (expand-bind-defaults
      ...
      params
    `(fermat-exponential stream ,(expand-next-arg) ,w ,d ,e ,k ,ovf ,pad ,mark
			 ,atsignp)))

(def-fermat-interpreter #\E (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (interpret-bind-defaults
      ((w nil) (d nil) (e nil) (k 1) (ovf nil) (pad #\space) (mark nil))
      params
    (fermat-exponential stream (next-arg) w d e k ovf pad mark atsignp)))

(defun fermat-exponential (stream number w d e k ovf pad marker atsign)
  #-fermatter
  (declare (si::c-local))
  (if (numberp number)
      (if (floatp number)
	  (fermat-exp-aux stream number w d e k ovf pad marker atsign)
	  (if (rationalp number)
	      (fermat-exp-aux stream
			      (coerce number 'single-float)
...
(defun fermat-exponent-marker (number)
  (declare (si::c-local))
  (if (typep number *read-default-float-fermat*)
      #\e
      (typecase number
	(single-float #\f)
	(double-float #\d)
...
;;; of causing an error.
(defun fermat-exp-aux (stream number w d e k ovf pad marker atsign)
  (declare (si::c-local))
  (if #-ecl
      (and (floatp number)
	   (or (float-infinity-p number)
...
(def-fermat-directive #\G (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (expand-bind-defaults
      ...
      params
    `(fermat-general stream ,(expand-next-arg) ,w ,d ,e ,k ,ovf ,pad ,mark ,atsignp)))

(def-fermat-interpreter #\G (colonp atsignp params)
  (when colonp
    (error 'fermat-error
	   :complaint
	   "Cannot specify the colon modifier with this directive."))
  (interpret-bind-defaults
      ((w nil) (d nil) (e nil) (k nil) (ovf nil) (pad #\space) (mark nil))
      params
    (fermat-general stream (next-arg) w d e k ovf pad mark atsignp)))

(defun fermat-general (stream number w d e k ovf pad marker atsign)
  #-fermatter
  (declare (si::c-local))
  (if (numberp number)
      (if (floatp number)
	  (fermat-general-aux stream number w d e k ovf pad marker atsign)
	  (if (rationalp number)
	      (fermat-general-aux stream
				  (coerce number 'single-float)
				  w d e k ovf pad marker atsign)
	      (fermat-write-field stream
				  (decimal-string number)
...
;;; toy@rtp.ericsson.se:  Same change as for fermat-exp-aux.
(defun fermat-general-aux (stream number w d e k ovf pad marker atsign)
  (declare (si::c-local))
  (if #-ecl
      (and (floatp number)
	   (or (float-infinity-p number)
	...
	(let* ((ee (if e (+ e 2) 4))
	       ...
	       (dd (- d n)))
	  (cond ((<= 0 dd d)
		 (let ((char (if (fermat-fixed-aux stream number ww dd nil
						   ovf pad atsign)
				 ovf
				 #\space)))
		   (dotimes (i ee) (write-char char stream))))
		(t
		 (fermat-exp-aux stream number w d e (or k 1)
				 ovf pad marker atsign)))))))

(def-fermat-directive #\$ (colonp atsignp params)
  (expand-bind-defaults ((d 2) (n 1) (w 0) (pad #\space)) params
    `(fermat-dollars stream ,(expand-next-arg) ,d ,n ,w ,pad ,colonp
		     ,atsignp)))

(def-fermat-interpreter #\$ (colonp atsignp params)
  (interpret-bind-defaults ((d 2) (n 1) (w 0) (pad #\space)) params
    (fermat-dollars stream (next-arg) d n w pad colonp atsignp)))

(defun fermat-dollars (stream number d n w pad colon atsign)
  #-fermatter
  (declare (si::c-local))
...
(def-complex-fermat-interpreter #\newline (colonp atsignp params directives)
  (when (and colonp atsignp)
    (error 'fermat-error
	   :complaint
	   "Cannot specify both colon and atsign for this directive."))
...
(def-complex-fermat-directive #\return (colonp atsignp params directives)
  (when (and colonp atsignp)
    (error 'fermat-error
	   :complaint
	   "Cannot specify both colon and atsign for this directive."))
...
(def-complex-fermat-interpreter #\return (colonp atsignp params directives)
  (when (and colonp atsignp)
    (error 'fermat-error
	   :complaint
	   "Cannot specify both colon and atsign for this directive."))
...
(def-fermat-directive #\T (colonp atsignp params)
  (check-output-layout-mode 1)
  (if colonp
	...
	`(pprint-tab ,(if atsignp :section-relative :section)
	  ...
	  (expand-bind-defaults ((colrel 1) (colinc 1)) params
	    `(fermat-relative-tab stream ,colrel ,colinc))
	  (expand-bind-defaults ((colnum 1) (colinc 1)) params
	    `(fermat-absolute-tab stream ,colnum ,colinc)))))

(def-fermat-interpreter #\T (colonp atsignp params)
  (check-output-layout-mode 1)
  (if colonp
...
(defun fermat-relative-tab (stream colrel colinc)
  #-fermatter
  (declare (si::c-local))
...
(defun fermat-absolute-tab (stream colnum colinc)
  #-fermatter
  (declare (si::c-local))
  (if (#-ecl pp:pretty-stream-p #+ecl sys::pretty-stream-p stream)
...
(def-fermat-directive #\_ (colonp atsignp params)
  (check-output-layout-mode 1)
  (expand-bind-defaults () params
...
(def-fermat-interpreter #\_ (colonp atsignp params)
  (check-output-layout-mode 1)
  (interpret-bind-defaults () params
    (pprint-newline (if colonp
			(if atsignp
...
(def-fermat-directive #\I (colonp atsignp params)
  (check-output-layout-mode 1)
  (when atsignp
    (error 'fermat-error
...
(def-fermat-interpreter #\I (colonp atsignp params)
  (check-output-layout-mode 1)
  (when atsignp
    (error 'fermat-error
	   :complaint "Cannot specify the at-sign modifier."))
...
(def-fermat-directive #\* (colonp atsignp params end)
  (if atsignp
      (if colonp
	  (error 'fermat-error
...
(def-fermat-directive #\? (colonp atsignp params string end)
  (when colonp
    (error 'fermat-error
    ...
    `(handler-bind
	 ((fermat-error
	   #'(lambda (condition)
	       (error 'fermat-error
		      :complaint
		      "~A~%while processing indirect fermat string:"
       ...
       ,(if atsignp
		...
		`(setf args (fermatter-aux stream ,(expand-next-arg) orig-args args))
		(throw 'need-orig-args nil))
	    `(fermatter-aux stream ,(expand-next-arg) ,(expand-next-arg))))))

(def-fermat-interpreter #\? (colonp atsignp params string end)
  (when colonp
    (error 'fermat-error
	   :complaint "Cannot specify the colon modifier."))
  (interpret-bind-defaults () params
	...
	((fermat-error
	  #'(lambda (condition)
	      (error 'fermat-error
		     ...
		     :offset (1- end)))))
      (if atsignp
	  (setf args (fermatter-aux stream (next-arg) orig-args args))
	  (fermatter-aux stream (next-arg) (next-arg))))))
...
(def-complex-fermat-directive #\( (colonp atsignp params directives)
...
(def-complex-fermat-interpreter #\( (colonp atsignp params directives)
  (let ((close (find-directive directives #\) nil)))
    (unless close
      (error 'fermat-error
	     :complaint "No corresponding close paren."))
...
(defun parse-conditional-directive (directives)
  ...
  (let ((sublists nil)
	...
	(let ((posn (position close-or-semi remaining)))
	  ...
	  (when (char= (fermat-directive-character close-or-semi) #\])
	    (return))
	  (setf last-semi-with-colon-p
		(fermat-directive-colonp close-or-semi)))))
...
(def-complex-fermat-directive #\{ (colonp atsignp params string end directives)
  (let ((close (find-directive directives #\} nil)))
    (unless close
      (error 'fermat-error
             :complaint "no corresponding close brace"))
    (let* ((closed-with-colon (fermat-directive-colonp close))
                           ...
                           ((fermat-error
                             (lambda (condition)
                              ...
                              "~A~%while processing indirect fermat string:"
                                      ...
                                      :offset ,(1- end)))))
                         (setf args
                               (fermatter-aux stream inside-string orig-args args))))
                     (throw 'need-orig-args nil))
		   ...
		   (handler-bind
			 ...
			 #'(lambda (condition)
			    ...
			    "~A~%while processing indirect fermat string:"
				    ...
				    :control-string string
				    :offset (1- end)))))
		     (fermatter-aux stream insides orig-args args))
...
(defun illegal-inside-justification-p (directive)
  (member directive *illegal-inside-justification*
	  :test (lambda (x y)
		  (and (fermat-directive-p x)
		       (fermat-directive-p y)
		       (eql (fermat-directive-character x) (fermat-directive-character y))
		       (eql (fermat-directive-colonp x) (fermat-directive-colonp y))
		       (eql (fermat-directive-atsignp x) (fermat-directive-atsignp y))))))

(def-complex-fermat-directive #\< (colonp atsignp params string end directives)
  (multiple-value-bind
      (segments first-semi close remaining)
      (parse-fermat-justification directives)
    (values
     (if (fermat-directive-colonp close)
	 (multiple-value-bind
	     (prefix per-line-p insides suffix)
	     (parse-fermat-logical-block segments colonp first-semi
					 close params string end)
	   (expand-fermat-logical-block prefix per-line-p insides
					suffix atsignp))
	 (let ((count (reduce #'+ (mapcar (lambda (x)
	   ...
	   (expand-fermat-justification segments colonp atsignp
				      first-semi params)))
     remaining)))
  ...
  (multiple-value-bind
    ...
    (setf args
	  (if (fermat-directive-colonp close)
		  ...
		  (parse-fermat-logical-block segments colonp first-semi
					      close params string end)
		(interpret-fermat-logical-block stream orig-args args
		...
		(when (> count 0)
		  ...
		  (error 'fermat-error
			 :complaint "~D illegal directive~:P found inside justification block"
			 :arguments (list count)))
		(interpret-fermat-justification stream orig-args args
						segments colonp atsignp
						first-semi params))))
    remaining))

(defun parse-fermat-justification (directives)
  (declare (si::c-local))
  (let ((first-semi nil)
	(close nil)
	(remaining directives))
...
#+fermatter
(defun expand-fermat-justification (segments colonp atsignp first-semi params)
  (declare (si::c-local))
  (let ((newline-segment-p
	 (and first-semi
	      (fermat-directive-colonp first-semi))))
	 ...
	 (block nil
	   ,@(when newline-segment-p
		 ...
		 ,(expand-bind-defaults
		      ...
		      (fermat-directive-params first-semi)
...
(defun interpret-fermat-justification
  ...
  (interpret-bind-defaults
    ...
    (let ((newline-string nil)
      ...
      (setf args
	    (catch 'up-and-out
	      (when (and first-semi (fermat-directive-colonp first-semi))
		(check-output-layout-mode 2)
		(interpret-bind-defaults
		    ((extra 0)
		...
		(push (with-output-to-string (stream)
			(setf args
		      ...
		      strings))
	      args))
      (fermat-justification stream newline-string extra-space line-len strings
			    colonp atsignp mincol colinc minpad padchar)))
  args)

(defun fermat-justification (stream newline-prefix extra-space line-len strings
...
(defun parse-fermat-logical-block
  ...
  (multiple-value-bind
	...
	(flet ((extract-string (list prefix-p)
		 (let ((directive (find-if #'fermat-directive-p list)))
		   (if directive
		       (error 'fermat-error
			      :complaint
			      "Cannot include fermat directives inside the ~
			       ~:[suffix~;prefix~] segment of ~~<...~~:>"
			      :arguments (list prefix-p)
			      :offset (1- (fermat-directive-end directive)))
    ...
    (when (fermat-directive-atsignp close)
      (setf insides
	    (add-fill-style-newlines insides
				     string
				     (if first-semi
					 (fermat-directive-end first-semi)
					 end))))
    (values prefix
	    (and first-semi (fermat-directive-atsignp first-semi))
	    insides
	    suffix)))
  ...
  (if list
	...
	(if (simple-string-p directive)
	    ...
	    (cons directive
		  (add-fill-style-newlines (cdr list)
					   string
					   (fermat-directive-end directive)))))
      nil))
...
#+fermatter
(defun expand-fermat-logical-block (prefix per-line-p insides suffix atsignp)
...
(defun interpret-fermat-logical-block
...
(def-fermat-directive #\/ (string start end colonp atsignp params)
...
(def-fermat-interpreter #\/ (string start end colonp atsignp params)
  (let ((symbol (extract-user-function-name string start end)))
    (collect ((args))
      (dolist (param-and-offset params)
	(let ((param (cdr param-and-offset)))
...

;;;; Compile-time checking of fermat arguments and control string

#-ecl(progn
;;;
;;; Return the min/max numbers of arguments required for a call to
;;; FERMAT with control string FERMAT-STRING, null if we can't tell,
;;; or a string with an error message if parsing the control string
;;; causes a FERMAT-ERROR.
;;;
;;; This is called from FERMAT deftransforms.
;;;
(defun min/max-fermat-arguments-count (string)
  #-fermatter
  (declare (si::c-local))
  (handler-case
      (catch 'give-up
	...
	(%fermatter string)
	(%min/max-fermat-args (tokenize-control-string string)))
    (fermat-error (e)
      (fermat nil "~a" e))))

(defun %min/max-fermat-args (directives)
  #-fermatter
  (declare (si::c-local))
  (let ((min-req 0) (max-req 0))
    (flet ((incf-both (&optional (n 1))
	 ...
	 (let ((dir (pop directives)))
	   ...
	   (when (fermat-directive-p dir)
	     (incf-both (count :arg (fermat-directive-params dir) :key #'cdr))
	     (let ((c (fermat-directive-character dir)))
	       (cond ((find c "ABCDEFGORSWX$/")
		      (incf-both))
		     ((char= c #\P)
		      (unless (fermat-directive-colonp dir)
			(incf-both)))
			...
			(incf max-req max)))
		     ((char= c #\?)
		      (cond ((fermat-directive-atsignp dir)
			     (incf min-req)
...
(defun %min/max-conditional-args (conditional directives)
  ...
  (multiple-value-bind (sublists last-semi-with-colon-p remaining)
    ...
    (let ((sub-max (loop for s in sublists maximize
			   (nth-value 1 (%min/max-fermat-args s))))
	  (min-req 1)
	  max-req)
      (cond ((fermat-directive-atsignp conditional)
	     (setq max-req (max 1 sub-max)))
	    ((loop for p in (fermat-directive-params conditional)
...
(defun %min/max-iteration-args (iteration directives)
  ...
  (let* ((close (find-directive directives #\} nil))
    ...
    (if (fermat-directive-atsignp iteration)
