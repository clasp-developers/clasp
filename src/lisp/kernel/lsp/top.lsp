    ...
    (do* ((icur (core:ihs-top) (core:ihs-prev icur))
      ...
      (let ((arg-str (with-output-to-string (sout)
                       (dotimes (i (length args))
                         (handler-case (format sout "~a " (elt args i))
                           (error (c)
        ...
        (let* ((fun (core:ihs-fun icur))
          ...
          (if (eq (function-name fun) 'cl:lambda)
              (format t "~4a ~a ~5d LAMBDA(~a)~%" icur filename lineno (subseq arg-str 0 160))
              (format t "~4a ~a ~5d (~a ~a)~%" icur filename lineno (function-name fun) (subseq arg-str 0 160)))))))
