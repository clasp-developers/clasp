...
(defun opcode (mnemonic)
  (let ((inst (assoc mnemonic +bytecode-ltv-ops+ :test #'equal)))
    (if inst
        (second inst)
...
(defmethod encode ((inst array-creator) stream)
  ...
  (let* ((packing-info (packing-info inst))
         ...
         (packing-code (getf +uaet-codes+ (second packing-info))))
...
(defun find-uaet-code (uaet)
  (getf +uaet-codes+ (second (%uaet-info uaet))))
...
(defun debug-info-opcode (mnemonic)
  (or (getf +debug-info-ops+ mnemonic)
