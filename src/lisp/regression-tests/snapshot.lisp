...
#+use-precise-gc
(test callback-snapshot
      (let ((binary (ext:argv 0))
            (snap-fname
              (namestring
               (translate-logical-pathname "sys:src;lisp;regression-tests;testcallback")))
            (ostream (make-string-output-stream)))
        (multiple-value-bind (stream dump-code)
            (ext:run-program binary
                             (list "--norc" "--base" "--feature" "ignore-extensions"
                                   "--non-interactive"
                                   "--eval"
                                   "(clasp-ffi:%defcallback snapshot-test-callback :int (x) (:int) (1+ x))"
                                   "--eval"
                                   (format nil "(ext:save-lisp-and-die \"~a\")" snap-fname))
                             :output ostream)
          (declare (ignore stream))
          (if (eql dump-code 0)
              (multiple-value-bind (stream code)
                  (ext:run-program
                   ...
                   (list "--snapshot" (format nil "~a" snap-fname)
                         "--eval"
                         "(ext:quit (clasp-ffi:%foreign-funcall-pointer (clasp-ffi:%get-callback 'snapshot-test-callback) :int 41 :int))")
                   :output ostream)
                (declare (ignore stream))
                (unless (eql code 42)
                  (write-string (get-output-stream-string ostream) *error-output*))
