...
(defun generate-dynamic-cast-code (fout analysis)
  (maphash (lambda (key enum) 
             (when (enum-in-hierarchy enum)
               (format fout "template <typename O> struct DynamicCast<~a*,O> {~%" key )
               (format fout "  static bool isA(O* client) {~%" key)
               ...
               (format fout "  };~%")
               (format fout "  static ~a* castOrNULL(O* client) {~%" key)
               (format fout "    if (DynamicCast<~a*,O>::isA(client)) {~%" key)
