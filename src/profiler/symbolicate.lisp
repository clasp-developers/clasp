...
(defun parse-symbol-table (stream entries)
  (declare (optimize (debug 3)))
  (loop for line = (read-line stream nil :eof)
        for linecount from 0
        until (eq line :eof)
        do (with-input-from-string (sin line)
             (let* ((hex-start (read-string sin))
                    (start (if (= (length hex-start) 0)
                               (error "Line ~a could not parse hex-start from: ~s" linecount line)
                               (parse-integer hex-start :radix 16)))
                    (hex-size (read-string sin))
                    (size (if (= (length hex-start) 0)
                               (error "Line ~a could not parse hex-size from: ~s" linecount line)
                               (parse-integer hex-size :radix 16)))
...
(defun load-symbol-table-into-entries (filename entries)
  (let ((fin (open filename :direction :input)))
    (unwind-protect
         (parse-symbol-table fin entries)
      (close fin))))
