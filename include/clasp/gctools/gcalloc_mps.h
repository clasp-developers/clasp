...
general_mps_allocation(const Header_s::BadgeStampWtagMtag& the_header, size_t size, mps_ap_t& allocation_point, ARGS&&... args) {
...
#ifdef DEBUG_MPS_SIZE
  {
    ...
    if (skip_size != allocate_size) {
      mps_addr_t againNextClient = obj_skip_debug_wrong_size((mps_addr_t)obj, header, (size_t)header->_stamp_wtag_mtag._value,
                                                             header->_stamp_wtag_mtag.stamp_(), allocate_size, skip_size,
