...
struct TaggedCast<core::BuiltinClosure *, FROM> {
   ...
   inline static bool isA(FromType tagged_client) {
     if ( tagged_generalp(tagged_client) ) {
       ...
       return (kindVal == gctools::GCKind<core::BuiltinClosure>::Kind
               || kindVal == gctools::GCKind<core::MacroClosure>::Kind);
     }
     return false;
   }
...
struct TaggedCast<core::Closure *, FROM> {
   ...
   inline static bool isA(FromType tagged_client) {
     if ( tagged_generalp(tagged_client) ) {
       ...
       int kindVal = header->kind();
       return (kindVal == gctools::GCKind<core::BuiltinClosure>::Kind
               || kindVal == gctools::GCKind<core::InstanceClosure>::Kind
               || kindVal == gctools::GCKind<llvmo::CompiledClosure>::Kind
               || kindVal == gctools::GCKind<core::MacroClosure>::Kind
               || kindVal == gctools::GCKind<core::SingleDispatchGenericFunctionClosure>::Kind
               || kindVal == gctools::GCKind<core::InterpretedClosure>::Kind
               //               || kindVal == gctools::GCKind<core::SingleDispatchGenericFunctionClosure::Kind
               );
     }
     return false;
