...
template <> struct gctools::GCInfo<core::SimpleVector_SPECIALIZE_ME_O> {
...
typedef template_SimpleVector<SimpleVector_SPECIALIZE_ME_O, SPECIALIZE_ME, AbstractSimpleVector_O>
...
public:
  SimpleVector_SPECIALIZE_ME_O(size_t length, value_type initialElement = value_type(), bool initialElementSupplied = false,
  ...
  static smart_ptr_type make(size_t length, value_type initialElement = value_type(), bool initialElementSupplied = false,
    ...
    auto bs = gctools::GC<my_type>::allocate_container(static_vector_p, length, initialElement, initialElementSupplied,
...
public: // make array
  MDArray_SPECIALIZE_ME_O(size_t rank, List_sp dimensions, Array_sp data, bool displacedToP, Fixnum_sp displacedIndexOffset)
      : TemplatedBase(rank, dimensions, data, displacedToP, displacedIndexOffset){};
  static smart_ptr_type make_multi_dimensional(List_sp dim_desig, simple_element_type initialElement, T_sp dataOrDisplacedTo,
    ...
    return gctools::GC<my_type>::allocate_container(false, rank, dim_desig, gc::As<Array_sp>(dataOrDisplacedTo), displacedToP,
...
public: // make array
  SimpleMDArray_SPECIALIZE_ME_O(size_t rank, List_sp dimensions, Array_sp data) : TemplatedBase(rank, dimensions, data){};
  static smart_ptr_type make_multi_dimensional(List_sp dim_desig, simple_element_type initialElement, T_sp data) {
    ASSERT(dim_desig.consp() || dim_desig.nilp());
    size_t rank;
    size_t arrayTotalSize = calculateArrayTotalSizeAndValidateDimensions(dim_desig, rank);
    LIKELY_if(data.nilp()) { data = SimpleVector_SPECIALIZE_ME_O::make(arrayTotalSize, initialElement, true); }
...
namespace core {
class ComplexVector_SPECIALIZE_ME_O
    : public template_Vector<ComplexVector_SPECIALIZE_ME_O, SimpleVector_SPECIALIZE_ME_O, ComplexVector_O> {
...
public: // make vector
  ComplexVector_SPECIALIZE_ME_O(size_t rank1, size_t dimension, T_sp fillPointer, Array_sp data, bool displacedToP,
  ...
  static smart_ptr_type make(size_t dimension, simple_element_type initialElement, bool initialElementSuppliedP, T_sp fillPointer,
    ...
    LIKELY_if(dataOrDisplacedTo.nilp()) dataOrDisplacedTo = simple_type::make(dimension, initialElement, initialElementSuppliedP);
    return gctools::GC<my_type>::allocate_container(false, 1 /*CRANK*/, dimension, fillPointer,
                                                    gc::As_unsafe<Array_sp>(dataOrDisplacedTo), displacedToP, displacedIndexOffset);
  }
};
}; // namespace core
