...
*/
  ...
  Function_O(claspFunction ptr)
    ...
    T_sp fdescInfo(int index) const {
      T_sp result((gctools::Tagged)this->fdesc()->gcrootsInModule->getTaggedIndex(this->fdesc()->sourcePosition_functionName_Index));
      return result;
    }
#endif
    CL_LISPIFY_NAME("core:functionName");
    CL_DEFMETHOD virtual T_sp functionName() const {
      Cons_sp cell((gctools::Tagged)this->fdesc()->gcrootsInModule->getTaggedIndex(this->fdesc()->sourcePathname_functionName_Index));
      return CONS_CDR(cell);
    }
    T_sp docstring() const {
      Cons_sp cell((gctools::Tagged)this->fdesc()->gcrootsInModule->getTaggedIndex(this->fdesc()->lambdaList_docstring_Index));
      return CONS_CDR(cell);
    }
